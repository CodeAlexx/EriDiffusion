// eridiffusion/crates/core/cuda/narrow_strided_backward.cu
// General scatter-add for narrow backward with dtype-correct atomic adds.
...
// Atomic add for FP16 using native atomicAdd if available, else CAS on 32-bit word
static __device__ inline void atomicAdd_f16(uint16_t* addr_h, uint16_t val_h)
{
#if __CUDA_ARCH__ >= 700
    // Native half atomicAdd (on newer architectures)
    __half* hp = reinterpret_cast<__half*>(addr_h);
    __half addv = *reinterpret_cast<__half*>(&val_h);
    atomicAdd(hp, addv);
#else
    // CAS fallback similar to bf16 but using half conversions
...
extern "C" __global__
void narrow_backward_scatter_add_kernel_v2(
    const uint8_t* __restrict__ grad_out,
    uint8_t* __restrict__ grad_in,
    int rank,
    const int64_t* __restrict__ out_shape,
    const int64_t* __restrict__ in_strides,    // elements
    const int64_t* __restrict__ out_strides,   // row-major elements
    int dim,
    int64_t start,
    int64_t elem_size,
    int dtype_tag,
    int64_t n_elements)
{
    ...
    default: {
        // Fallback: plain byte-wise copy (no overlap in narrow scatter)
...
extern "C" int eri_narrow_backward_scatter_add_launch_v2(
    const void* grad_out,
    void* grad_in,
    int rank,
    ...
    void* stream_void)
{
    ...
    narrow_backward_scatter_add_kernel_v2<<<blocks, threads, 0, stream>>>(
...
// Backward-compatible symbol name expected by Rust FFI wrappers
extern "C" int eri_narrow_backward_scatter_add_launch(
    const void* grad_out,
    void* grad_in,
    int rank,
    const int64_t* out_shape_host,
    ...
    void* stream_void)
{
    return eri_narrow_backward_scatter_add_launch_v2(
        grad_out, grad_in, rank, out_shape_host, in_strides_host, out_strides_host,
        dim, start, elem_size, dtype_tag, n_elements, stream_void
    );
}
