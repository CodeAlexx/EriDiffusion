...
fn test_tensor_ops_cosine_similarity() {
    ...
    let sim_value = sim.item::<f32>().unwrap();
    ...
    let sim_value2 = sim2.item::<f32>().unwrap();
...
fn test_scaled_dot_product_attention() {
    ...
    let query = Tensor::randn(0f32, 1f32, Shape::from(vec![batch_size, num_heads, seq_len, head_dim]), &device).unwrap();
    let key = Tensor::randn(0f32, 1f32, Shape::from(vec![batch_size, num_heads, seq_len, head_dim]), &device).unwrap();
    let value = Tensor::randn(0f32, 1f32, Shape::from(vec![batch_size, num_heads, seq_len, head_dim]), &device).unwrap();
...
fn test_attention_with_mask() {
    ...
    let query = Tensor::ones(Shape::from(vec![batch_size, num_heads, seq_len, head_dim]), DType::F32, &device).unwrap();
    let key = Tensor::ones(Shape::from(vec![batch_size, num_heads, seq_len, head_dim]), DType::F32, &device).unwrap();
    ...
    let mask = Tensor::from_vec(mask_values, Shape::from(vec![1, 1, seq_len, seq_len]), &device).unwrap();
...
fn test_edge_cases() {
    ...
    // Empty tensor
    let empty = Tensor::zeros(Shape::from(vec![0]), DType::F32, &device).unwrap();
    assert!(empty.normalize(0).is_ok());
    
    // Single element
    let single = Tensor::from_vec(vec![42.0f32], 1, device.clone()).unwrap();
    let normalized = single.normalize(0).unwrap();
    assert_eq!(normalized.item::<f32>().unwrap(), 1.0);
}
#![cfg(all())]
