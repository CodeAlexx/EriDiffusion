...
pub trait MaybeAsync {
    ...
    fn execute_sync(self) -> anyhow::Result<Self::Output>;
    ...
    {
        Box::pin(async move {
            let res_anyhow = task::spawn_blocking(move || self.execute_sync())
                .await
                .map_err(|e| Error::Runtime(format!("Task join error: {}", e)))?;
            res_anyhow.map_err(|e| Error::Runtime(format!("Task error: {}", e)))
        })
    }
}
...
impl<T> AsyncWrapper<T> {
    ...
    where
        F: FnOnce(&T) -> anyhow::Result<R> + Send + 'static,
        T: Send + Sync + 'static,
        R: Send + 'static,
    {
    ...
    pub async fn run_mut<F, R>(&self, f: F) -> anyhow::Result<R>
    where
        F: FnOnce(&T) -> anyhow::Result<R> + Send + 'static,
        T: Send + Sync + 'static,
        R: Send + 'static,
...
impl<T> AsyncBatch<T> {
    ...
    pub async fn execute_all<F, R>(self, f: F) -> anyhow::Result<Vec<R>>
    where
    ...
    pub async fn execute_limited<F, R>(self, f: F, limit: usize) -> anyhow::Result<Vec<R>>
    where
        F: Fn(T) -> Pin<Box<dyn Future<Output = Result<R>> + Send>> + Clone,
        T: Send + 'static,
        R: Send + 'static,
    {
        ...
        stream.await.map_err(anyhow::Error::new)
    }
}
...
impl AsyncIO {
    ...
    pub async fn read_file(path: &std::path::Path) -> anyhow::Result<Vec<u8>> {
    ...
    pub async fn write_file(path: &std::path::Path, data: &[u8]) -> anyhow::Result<()> {
        Ok(tokio::fs::write(path, data)
            .await
            .map_err(|e| Error::Io(e))?)
    }
    
    /// Create directory asynchronously
    pub async fn create_dir_all(path: &std::path::Path) -> anyhow::Result<()> {
