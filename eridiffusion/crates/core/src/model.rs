...
impl std::fmt::Debug for ModelInputs {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ModelInputs")
            .field("latents_shape", &self.latents.shape())
            .field("timestep_shape", &self.timestep.shape())
            .field("encoder_hidden_states", &self.encoder_hidden_states.as_ref().map(|t| t.shape()))
            .field("pooled_projections", &self.pooled_projections.as_ref().map(|t| t.shape()))
            .field("attention_mask", &self.attention_mask.as_ref().map(|t| t.shape()))
...
impl std::fmt::Debug for ModelOutput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ModelOutput")
            .field("sample_shape", &self.sample.shape())
            .field("additional_keys", &self.additional.keys().collect::<Vec<_>>())
            .finish()
    }
}
...
pub trait DiffusionModel: Send + Sync {
    ...
    async fn load_pretrained(&mut self, path: &Path) -> anyhow::Result<()>;
    
    /// Save model weights
    async fn save_pretrained(&self, path: &Path) -> anyhow::Result<()>;
    ...
    fn forward_with_gradient_checkpointing(&self, inputs: &ModelInputs) -> anyhow::Result<ModelOutput> {
...
pub struct ModelLoadConfig {
    pub device: Device,
    #[serde(with = "crate::dtype::dtype_serde")]
    pub dtype: DType,
