...
impl ModelDispatch {
    ...
    pub fn from_architecture(arch: ModelArchitecture, device: crate::Device) -> anyhow::Result<Self> {
        match arch {
...
impl DiffusionModel for ModelDispatch {
    ...
    fn to_device(&mut self, device: &crate::Device) -> anyhow::Result<()> {
        match self {
    ...
    async fn load_pretrained(&mut self, path: &std::path::Path) -> anyhow::Result<()> {
        match self {
    ...
    async fn save_pretrained(&self, path: &std::path::Path) -> anyhow::Result<()> {
        match self {
    ...
    fn state_dict(&self) -> anyhow::Result<HashMap<String, Tensor>> {
        match self {
    ...
    fn load_state_dict(&mut self, state_dict: HashMap<String, Tensor>) -> anyhow::Result<()> {
        match self {
...
impl NetworkAdapter for NetworkDispatch {
    fn forward(&self, x: &Tensor, inputs: &ModelInputs) -> anyhow::Result<NetworkOutput> {
        match self {
    ...
    fn merge_weights(&self, base_weights: &mut HashMap<String, Tensor>) -> anyhow::Result<()> {
        match self {
