...
pub trait Plugin: Send + Sync + Any {
    ...
    /// Handle events
    async fn on_event(&mut self, _event: PluginEvent) -> anyhow::Result<()> {
        Ok(())
    }
}
...
impl PluginRegistry {
    ...
    pub async fn load_plugin(&self, path: &Path) -> anyhow::Result<Uuid> {
        // Verify the plugin file exists
        if !path.exists() {
            return Err(Error::Plugin(format!("Plugin file not found: {:?}", path)).into());
        }
    ...
    /// Unload a plugin
    pub async fn unload_plugin(&self, id: Uuid) -> anyhow::Result<()> {
        // Get the plugin
    ...
    /// Trigger a hook
    pub async fn trigger_hook(&self, hook: HookType, event: PluginEvent) -> anyhow::Result<()> {
...
pub fn initialize_plugin_system() -> anyhow::Result<()> {
