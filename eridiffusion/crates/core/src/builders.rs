...
impl ModelConfigBuilder {
    ...
    pub fn custom<T: serde::Serialize>(mut self, key: &str, value: T) -> anyhow::Result<Self> {
    ...
    pub fn build(self) -> anyhow::Result<ModelConfig> {
        ...
        Ok(ModelConfig {
            architecture: self.architecture
                .ok_or_else(|| Error::Config("Architecture not specified".into()))?,
            device,
            precision: self.precision.unwrap_or(Precision::Float16),
            model_path: self.model_path
                .ok_or_else(|| Error::Config("Model path not specified".into()))?,
...
impl TrainingConfigBuilder {
    ...
    pub fn build(self) -> anyhow::Result<TrainingConfig> {
        // Validate required fields
        let model_config = self.model_config
            .ok_or_else(|| Error::Config("Model configuration not specified".into()))?;
...
impl InferenceConfigBuilder {
    ...
    pub fn build(self) -> anyhow::Result<InferenceConfig> {
        let model_config = self.model_config
            .ok_or_else(|| Error::Config("Model configuration not specified".into()))?;
