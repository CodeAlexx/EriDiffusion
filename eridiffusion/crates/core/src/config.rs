...
use crate::{Error, ModelArchitecture, NetworkType};
...
pub trait Config: Serialize + for<'de> Deserialize<'de> + Send + Sync {
    ...
    fn merge(&mut self, other: &Value) -> anyhow::Result<()>;
    
    /// Get as JSON value
    fn as_value(&self) -> anyhow::Result<Value> {
...
impl ConfigLoader {
    ...
    pub fn load<T: Config>(&self, filename: &str) -> anyhow::Result<T> {
        ...
        let config: T = match file_path.extension().and_then(|s| s.to_str()) {
            Some("yaml") | Some("yml") => serde_yaml::from_str(&content)
                .map_err(|e| anyhow::anyhow!(format!("Failed to parse YAML: {}", e)))?,
            Some("json") => serde_json::from_str(&content)
                .map_err(|e| anyhow::anyhow!(format!("Failed to parse JSON: {}", e)))?,
            Some("toml") => toml::from_str(&content)
                .map_err(|e| anyhow::anyhow!(format!("Failed to parse TOML: {}", e)))?,
            _ => return Err(Error::Config("Unsupported config format".into()).into()),
        };
    ...
    fn find_config_file(&self, filename: &str) -> anyhow::Result<PathBuf> {
        ...
        Err(Error::Config(format!("Config file not found: {}", filename)).into())
    }
    
    /// Apply environment variable overrides
    fn apply_env_overrides<T: Config>(&self, config: T) -> anyhow::Result<T> {
        ...
        serde_json::from_value(value)
            .map_err(|e| anyhow::anyhow!(format!("Failed to apply env overrides: {}", e)))
    }
    
    /// Set a value at a dotted path
    fn set_value_at_path(&self, value: &mut Value, path: &str, new_value: &str) -> anyhow::Result<()> {
...
pub struct UniversalConfig {
    ...
    /// Attention runtime configuration (optional)
    #[serde(default)]
    pub attention: Option<AttentionRuntimeConfig>,
}

/// Attention module configuration
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AttentionRuntimeConfig {
    /// Implementation selector: "flash" or "sdpa"
    #[serde(rename = "impl", default)]
    pub impl_sel: Option<String>,
    /// Whether QKV is provided in packed format
    #[serde(default)]
    pub packed_qkv: Option<bool>,
}
...
impl Config for UniversalConfig {
    fn validate(&self) -> anyhow::Result<()> {
        ...
        if self.job.name.is_empty() {
            return Err(Error::Config("Job name cannot be empty".into()).into());
        }
        
        // Validate model config
        if self.model.pretrained_model_name_or_path.is_empty() {
            return Err(Error::Config("Model path cannot be empty".into()).into());
        }
        
        // Validate training config if present
        if let Some(ref training) = self.training {
            if training.batch_size == 0 {
                return Err(Error::Config("Batch size must be greater than 0".into()).into());
            }
            if training.learning_rate <= 0.0 {
                return Err(Error::Config("Learning rate must be positive".into()).into());
            }
        }
