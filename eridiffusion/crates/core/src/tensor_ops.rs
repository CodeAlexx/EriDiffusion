...
use crate::tensor_narrow::TensorNarrowExt;
...
impl FusedKernel {
    ...
    pub fn apply(tensor: &Tensor, op: FusedOp) -> anyhow::Result<Tensor> {
        match op {
            FusedOp::AddScalar(scalar) => {
                tensor.add(&Tensor::from_vec(scalar, tensor.device())?)
                    .map_err(|e| Error::TensorOp(e))
            }
            FusedOp::MulScalar(scalar) => {
                tensor.mul(&Tensor::from_vec(scalar, tensor.device())?)
                    .map_err(|e| Error::TensorOp(e))
            }
            ...
            FusedOp::Gelu => {
                ...
                let sigmoid = flame_core::nn::ops::sigmoid(&sigmoid_input)?;
                tensor.mul(&sigmoid)
                    .map_err(|e| Error::TensorOp(e))
            }
        }
...
impl BatchedOps {
    ...
    pub fn apply_sequence(tensor: &Tensor, ops: &[FusedOp]) -> anyhow::Result<Tensor> {
    ...
    pub fn batched_matmul(a: &Tensor, b: &Tensor) -> anyhow::Result<Tensor> {
...
impl MemoryEfficientOps {
    ...
    pub fn add_inplace(tensor: &mut Tensor, other: &Tensor) -> anyhow::Result<()> {
    ...
    pub fn streaming_reduce(
        ...
        if dim_size <= chunk_size {
            ...
            return tensor.sum_keepdim(dim as isize).map_err(|e| Error::TensorOp(e));
        }
        ...
        while start < dim_size {
            ...
            let chunk = tensor.narrow_general_cuda(dim, start, end - start)?;
            let chunk_sum = chunk.sum_keepdim(dim as isize)?;
        ...
        result.ok_or_else(|| Error::TensorOp("Empty tensor".into()))
    }
}
...
impl<'a> TensorIterator<'a> {
    ...
    fn extract_element(&self, indices: &[usize]) -> anyhow::Result<f32> {
        // Build index tensors for gathering
        let mut current_tensor = self.tensor.clone();
        
        for (dim, &idx) in indices.iter().enumerate() {
            // Select along dimension
            current_tensor = current_tensor.narrow_general_cuda(dim, idx, 1)?
                .squeeze(dim)?;
        }
        
        // Convert scalar to f32
        current_tensor.item::<f32>()
    }
}
...
mod tests {
    ...
    fn test_fused_ops() {
        ...
        let tensor = Tensor::randn(0.0f32, 1.0f32, Shape::from(vec![2, 3]), &device).unwrap();
