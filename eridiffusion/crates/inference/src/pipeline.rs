...
impl InferencePipeline {
    ...
    async fn run_inference(
        ...
        for (i, &timestep) in timesteps[start_idx..].iter().enumerate() {
            ...
            let inputs = ModelInputs {
                ...
                timestep: Tensor::from_vec(vec![timestep as f32], [1], &self.device)?,
    ...
    async fn encode_prompt(
        ...
        let text_encoder = self.text_encoder.as_ref()
            .ok_or_else(|| Error::Model("Text encoder not loaded".into()))?;
        ...
        let negative_embeddings = Tensor::zeros(Shape::from(vec![1, seq_len, embedding_dim]), DType::F32,
            &self.device,
        )?;
...
use flame_core::Shape;
    ...
    /// Encode image to latents
    async fn encode_image(&self, image: &Tensor) -> anyhow::Result<Tensor> {
    ...
    async fn decode_latents(&self, latents: &Tensor) -> anyhow::Result<Tensor> {
    ...
    /// Add noise to latents
    async fn add_noise(&self, latents: &Tensor, timestep: usize) -> anyhow::Result<Tensor> {
    ...
    async fn prepare_masked_latents(
        ...
        let latent_mask = self.resize_mask(mask, latents.dim(2), latents.dim(3))?;
    ...
    async fn apply_adapters(&self, latents: &Tensor) -> anyhow::Result<HashMap<String, Tensor>> {
    ...
    /// Resize mask
    fn resize_mask(&self, mask: &Tensor, height: usize, width: usize) -> anyhow::Result<Tensor> {
        ...
        let batch_size = if current_shape.len() > 2 { current_shape.dim(0) } else { 1 };
        let channels = if current_shape.len() > 3 { current_shape.dim(1) } else { 1 };
...
trait Scheduler: Send + Sync {
    ...
    fn step(&self, noise_pred: &Tensor, timestep: usize, sample: &Tensor) -> anyhow::Result<Tensor>;
    fn add_noise(&self, sample: &Tensor, noise: &Tensor, timestep: usize) -> anyhow::Result<Tensor>;
    fn init_noise_sigma(&self) -> f32;
}

/// Create scheduler
fn create_scheduler(scheduler_type: SchedulerType, config: &InferenceConfig) -> anyhow::Result<Box<dyn Scheduler>> {
