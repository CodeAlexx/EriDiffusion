...
impl InferenceClient {
    ...
    pub async fn check_status(&self, request_id: &str) -> anyhow::Result<RequestStatus> {
    ...
    /// Get server metrics
    pub async fn get_metrics(&self) -> anyhow::Result<ServerMetrics> {
    ...
    async fn execute_request(
        ...
        Err(last_error.unwrap_or_else(|| Error::Runtime("Request failed".into())))
    }
...
impl AsyncInferenceClient {
    ...
    pub fn new(base_url: &str, num_connections: usize) -> anyhow::Result<Self> {
...
pub mod utils {
    ...
    pub fn decode_image(base64_str: &str) -> anyhow::Result<Tensor> {
        let bytes = general_purpose::STANDARD
            .decode(base64_str)
            .map_err(|e| Error::Runtime(e.to_string()))?;
        
        // Would decode image bytes to tensor; create on default GPU (shared Device is recommended upstream)
        let dev = flame_core::CudaDevice::new(0)?;
        Ok(Tensor::zeros(flame_core::Shape::from_dims(&[3, 512, 512]), dev)?)
    }
    
    /// Encode tensor to base64 image
    pub fn encode_image(tensor: &Tensor) -> anyhow::Result<String> {
        ...
        // Convert to u8
        let tensor_u8 = (tensor * 255.0)?.to_dtype(ImageBuffer};
        
        let img: image::RgbImage = if c == 3 {
            // RGB image
            ImageBuffer::from_raw(w as u32, h as u32, raw_data)
                .ok_or_else(|| Error::Conversion("Failed to create image from tensor".into()))?
        } else {
