...
impl ModelOptimizer {
    ...
    pub async fn optimize_model<M: Module>(
        ...
        model: M,
        device: &Device,
    ) -> anyhow::Result<OptimizedModel<M>> {
    ...
    fn apply_fusion<M>(&self, model: &mut OptimizedModel<M>) -> anyhow::Result<()> {
        // Conv+BN fusion
    ...
    async fn static_quantization<M>(
        ...
        if cal_data.is_empty() {
            return Err(Error::InvalidInput("No calibration data".into()));
        }
    ...
    fn apply_memory_optimization<M>(&self, model: &mut OptimizedModel<M>) -> anyhow::Result<()> {
