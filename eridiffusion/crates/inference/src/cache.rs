...
impl InferenceCache {
    ...
    /// Put in cache
    pub async fn put(&self, key: CacheKey, value: CachedValue) -> anyhow::Result<()> {
        // Add to memory cache
        {
            let mut cache = self.memory_cache.write().await;
        ...
        Ok(Self {
            cache: LruCache::new(
                NonZeroUsize::new(capacity)
                    .ok_or_else(|| Error::InvalidInput("Invalid cache capacity".into()))?
            ),
        })
    }
}
...
impl DiskCache {
    async fn new(config: &CacheConfig) -> anyhow::Result<Self> {
    ...
    async fn get(&self, key: &CacheKey) -> anyhow::Result<Option<CachedValue>> {
        let hash = key.hash();
        ...
        if self.index.contains_key(&hash) {
            ...
            if path.exists() {
                ...
                let value = CachedValue {
                    tensor: Tensor::zeros(Shape::from(vec![1]), DType::F32, &metadata: HashMap::new(),
    ...
    async fn put(&mut self, key: CacheKey, value: CachedValue) -> anyhow::Result<()> {
        let hash = key.hash();
        let path = std::path::Path::new(&self.config.disk_cache_path)
            .join(format!("{}.cache", hash));
    ...
    async fn load_index(&mut self) -> anyhow::Result<()> {
