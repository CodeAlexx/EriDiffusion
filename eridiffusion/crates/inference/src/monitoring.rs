...
impl PerformanceMonitor {
    ...
    /// Start span
    fn start_span(&self, name: &str) -> Option<SpanHandle> {
        self.tracer.as_ref().map(|t| t.start_span(name))
    }
    
    /// Start profiling
    fn start_profiling(&self, name: &str) -> Option<ProfileHandle> {
    ...
    /// Export metrics
    pub async fn export_metrics(&self) -> anyhow::Result<String> {
