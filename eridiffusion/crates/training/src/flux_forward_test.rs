...
impl Default for TestConfig {
    fn default() -> Self {
        Self {
            ...
            device: Device::Cuda(0), // This is eridiffusion_core::Device, not test_image_size: 1024,
            num_timesteps: 1000,
            verbose: true,
        }
    }
...
impl FluxForwardTester {
    ...
    /// Convert Device to Flame CUDA device
    fn to_Device::Cuda(id) => Device::Cuda(*id)
    ...
    /// Test 1: Model Loading
    fn test_model_loading(&mut self) -> anyhow::Result<()> {
        println!("📦 Test 1: Model Loading");
        
        let start = Instant::now();
        let device = self.to_DType::F32, &device);
        ...
        if self.config.vae_path.exists() {
            let vae_vb = unsafe { 
                WeightLoader::from_mmaped_safetensors(&[&self.config.vae_path], DType::F32, &device)?
            };
    ...
    /// Test 2: Forward Pass Shapes
    fn test_forward_pass_shapes(&mut self) -> anyhow::Result<()> {
        println!("\n📐 Test 2: Forward Pass Shapes");
        
        let model = self.model.as_ref().ok_or(Error::Training("Model not loaded".into()))?;
        let batch_size = self.config.test_batch_size;
        let device = self.to_1.0, Shape::from(vec![batch_size, 16, latent_size, latent_size]), // Flux uses 16 latent channels
            &device
        )?;
        
        // Create text embeddings (T5-XXL dimensions)
        let text_embeddings = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, 512, 4096]), // T5-XXL: 512 tokens, 4096 dim
            &device
        )?;
        
        // Create pooled embeddings (CLIP dimensions)
        let pooled_embeddings = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, 768]), // CLIP-L pooled: 768 dim
            &device
        )?;
    ...
    /// Test 3: Gradient Flow
    fn test_gradient_flow(&mut self) -> anyhow::Result<()> {
        println!("\n🔄 Test 3: Gradient Flow");
        
        let model = self.model.as_ref().ok_or(Error::Training("Model not loaded".into()))?;
        let var_map = self.var_map.as_ref().ok_or(Error::Training("VarMap not loaded".into()))?;
        let device = self.to_1.0, Shape::from(vec![batch_size, 16, latent_size, latent_size]), &device
        )?;
        
        let text_embeddings = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, 512, 4096]), &device
        )?;
        
        let pooled_embeddings = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, 768]), &device
        )?;
        ...
        let target = Tensor::randn(0.0, 1.0, output.shape(), device.clone())?;
        let loss = output.sub(&target)?.sqr()?.mean_all()?;
        
        println!("  ✓ Loss computed: {:.6}", loss.item::<f32>()?);
        
        // Backward pass
        let start = Instant::now();
        loss.flame_core::autograd_v3::backward()?;
        ...
        if !has_gradients {
            return Err(Error::Training("No gradients found!".into()));
        }
    ...
    /// Test 4: Memory and Performance
    fn test_memory_performance(&mut self) -> anyhow::Result<()> {
        println!("\n💾 Test 4: Memory and Performance");
        
        let model = self.model.as_ref().ok_or(Error::Training("Model not loaded".into()))?;
        let device = self.to_2, 4];
        let latent_size = self.config.test_image_size / 8;
        
        for batch_size in batch_sizes {
            let latents = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, 16, latent_size, latent_size]), &device
            )?;
    ...
    /// Test 5: Different Input Configurations
    fn test_input_variations(&mut self) -> anyhow::Result<()> {
        ...
        for (i, t_values) in test_timesteps.iter().enumerate() {
            ...
            let output_mean = output.mean_all()?.item::<f32>()?;
            let output_std = output.var_all()?.item::<f32>()?.sqrt();
        ...
        // Empty text (zeros)
        let empty_text = Tensor::zeros(Shape::from(vec![batch_size, 512, 4096]), DType::F32, &device)?;
    ...
    /// Test 6: Gradient Accumulation
    fn test_gradient_accumulation(&mut self) -> anyhow::Result<()> {
        println!("\n📊 Test 6: Gradient Accumulation");
        ...
        for var in all_vars.iter() {
            // var.zero_grad() removed - handle gradients manually
        }
        ...
        for step in 0..accumulation_steps {
            ...
            accumulated_loss += scaled_loss.item::<f32>()?;
            
            // Backward
            scaled_loss.flame_core::autograd_v3::backward()?;
            
            println!("  ✓ Step {}/{}: loss={:.6}", 
                     step + 1, accumulation_steps, loss.item::<f32>()?);
        }
        ...
        for var in all_vars.iter() {
            if let Ok(grad) = var.grad() {
                ...
                let norm = grad_flat.sqr()?.sum()?.item::<f32>()?;
                grad_norm += norm;
            }
        }
    ...
    /// Test 7: Mixed Precision
    fn test_mixed_precision(&mut self) -> anyhow::Result<()> {
        println!("\n🎯 Test 7: Mixed Precision");
        
        let device = self.to_DType::F16, &device);
        ...
        let target = Tensor::randn(0.0, 1.0, output.shape(), device.clone())?.to_dtype(DType::F16)?;
        let loss = output.sub(&target)?.sqr()?.mean_all()?;
        
        // Loss should be computed in FP32 for stability
        let loss_fp32 = loss.to_dtype(DType::F32)?;
        loss_fp32.flame_core::autograd_v3::backward()?;
        
        println!("  ✓ Mixed precision loss: {:.6}", loss_fp32.item::<f32>()?);
...
fn count_parameters(var_map: &VarMap) -> anyhow::Result<usize> {
    let mut total = 0;
    let all_vars = var_map.all_vars();
    for var in all_vars.iter() {
        total += var.elem_count();
    }
    Ok(total)
}

/// Main test runner
pub fn run_forward_tests(config: TestConfig) -> anyhow::Result<()> {
...
use flame_core::Shape;
    ...
    fn test_forward_pass() -> anyhow::Result<()> {
        let config = TestConfig {
            device: Device::cuda(0)?, // Use CPU for testing
            test_batch_size: 1,
