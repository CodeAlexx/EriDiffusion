                    ...
                    if let Some(baseline) = self.baseline {
                        if !self.is_better(*current, baseline) {
                            ...
                            if self.wait >= self.patience {
                                ...
                                println!(
                                    "Early stopping triggered at epoch {} (baseline not met)",
                                    epoch
                                );
                            }
                            return Ok(());
                    ...
                    if let Some(best) = self.best_value {
                        if self.is_better(*current, best) {
                            ...
                            if self.wait >= self.patience {
                                self.stopped_epoch = Some(epoch);
                                println!(
                                    "Early stopping triggered at epoch {} (no improvement)",
                                    epoch
                                );
                            }
                        }
    ...
    async fn on_event(
        ...
        match event {
            CallbackEvent::EpochEnd(_) => {
                let metric_value =
                    self.metric.as_ref().and_then(|m| context.metrics.get(m)).copied();
...
impl TensorBoardCallback {
    pub fn new(log_dir: &str, log_interval: usize) -> Self {
        Self { log_dir: log_dir.to_string(), log_interval }
    }
}
...
where
    F: Fn(CallbackEvent, &CallbackContext) -> anyhow::Result<()> + Send + Sync,
{
    name: String,
    on_event_fn: F,
}
    ...
    F: Fn(CallbackEvent, &CallbackContext) -> anyhow::Result<()> + Send + Sync,
{
    pub fn new(name: &str, on_event_fn: F) -> Self {
        Self { name: name.to_string(), on_event_fn }
    }
}
...
where
    F: Fn(CallbackEvent, &CallbackContext) -> anyhow::Result<()> + Send + Sync + 'static,
{
    async fn on_event(
        &mut self,
        event: CallbackEvent,
...
impl CSVLogger {
    pub fn new(file_path: &str) -> Self {
        Self { file_path: file_path.to_string(), headers_written: false }
    }
}
