...
pub fn run_simple_flux_test() -> anyhow::Result<()> {
    ...
    let vb = WeightLoader::from_varmap(&var_map, DType::F32, &device);
    ...
    // Create dummy inputs
    let latents = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, 16, latent_size, latent_size]), // Flux uses 16 latent channels
        &device
    )?;
    
    // Create text embeddings (T5-XXL dimensions)
    let text_embeddings = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, 64, 4096]), // Reduced sequence length for testing
        &device
    )?;
...
use flame_core::Shape;
    ...
    #[test]
    fn test_simple_flux() -> anyhow::Result<()> {
