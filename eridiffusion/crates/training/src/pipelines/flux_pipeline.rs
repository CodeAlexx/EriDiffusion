...
impl TrainingPipeline for FluxPipeline {
    ...
    fn prepare_batch(&self, batch: &DataLoaderBatch) -> Result<PreparedBatch> {
        ...
        let control_latents = if let Some(control_images) = batch.metadata.get("control_images") {
            if let Ok(control_tensor) = serde_json::from_value::<Tensor>(control_images.clone()) {
                if let Some(vae) = &self.vae {
                    debug!("Encoding control images to latents");
                    // Control images should be normalized to [-1, 1] range
                    let normalized_control = control_tensor.affine(2.0, -1.0)?;
        ...
        metadata.insert("has_control".to_string(), serde_json::json!(control_latents.is_some()));
    ...
    fn encode_prompts(&self, prompts: &[String], _model: &dyn DiffusionModel) -> Result<PromptEmbeds> {
        if let Some(encoders) = &self.text_encoders {
            ...
            return Err(Error::Config("Text encoders not configured. Flux requires CLIP and T5-XXL encoders.".to_string()));
        }
    }
    ...
    fn add_noise(
        ...
        let noisy = latents.broadcast_mul(&one_minus_t)?.add(&noise.broadcast_mul(&t_expanded)?)?;
    ...
    fn get_model_inputs(
        ...
        additional.insert("latent_size".to_string(), 
            Tensor::new(packed_latents.dims()[1] as i64 / if has_control { 2 } else { 1 }, device)?);
    ...
    fn get_scheduler_config(&self) -> HashMap<String, serde_json::Value> {
        ...
        config.insert("shift".to_string(), 
            serde_json::json!(self.flow_matching.config.shift));
...
impl FluxPipeline {
    ...
    fn apply_dynamic_shifting(&self, t: &Tensor, latents: &Tensor) -> Result<Tensor> {
        ...
        let shift_term = t.broadcast_mul(&one_minus_t)?.affine(shift_factor as f64, 0.0)?;
