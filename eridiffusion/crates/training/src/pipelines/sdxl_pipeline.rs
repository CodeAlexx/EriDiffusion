...
impl DDPMScheduler {
    ...
    pub fn init(&mut self, device: &Device) -> Result<()> {
        ...
        let betas = match self.beta_schedule.as_str() {
            "linear" => {
                ...
                let betas: Vec<f32> = (0..num_steps)
                    ...
                    .collect();
                Tensor::new(betas, &candle_device)?
            }
            "scaled_linear" => {
        ...
        let ones = Tensor::ones(betas.shape(), betas.dtype(), &candle_device)?;
    ...
    pub fn sample_timesteps(&self, batch_size: usize, device: &Device) -> Result<Tensor> {
        ...
        Tensor::new(timesteps, &candle_device)
...
impl SDXLPipeline {
    ...
    fn encode_size_conditioning(&self, sizes: &[(u32, u32)], device: &candle_core::Device) -> Result<Tensor> {
