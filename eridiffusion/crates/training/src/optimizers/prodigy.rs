...
impl Prodigy {
    ...
    fn init_state(&mut self, var_id: String, shape: &[usize], device: &Device) -> anyhow::Result<()> {
    ...
    fn update_d_estimate(&mut self) -> anyhow::Result<()> {
...
impl CanOptimizer for Prodigy {
    ...
    fn step(&mut self, grads: &var) in self.var_map.all_vars().iter().enumerate() {
    ...
    fn backward_step(&mut self, loss: &Tensor) -> anyhow::Result<()> {
        let grads = loss.flame_core::autograd_v3::backward()?;
        self.step(&grads) // TODO: Use gradient_map instead of individual tensor
    }
}
...
pub fn create_prodigy_preset(
    ...
    let config = match preset {
        ...
        _ => return Err(eridiffusion_core::Error::Config(format!("Unknown Prodigy preset: {}", preset)).into()),
    };
...
//! Prodigy optimizer (Flame-compatible helper types)
//! Based on "Prodigy: An Expeditiously Adaptive Parameter-Free Learner"
//! https://arxiv.org/abs/2306.06101

use std::collections::HashMap;
use flame_core::{Tensor, DType, Device, Parameter};

/// Minimal Prodigy state container. This module provides a stable interface
/// used elsewhere in the crate (configs, monitoring, presets). The actual
/// training-time optimizer is implemented as `ProdigyOptimizer` in
/// `crate::optimizer` to match the project's `Optimizer` trait.
pub struct Prodigy {
    // Hyperparameters
    d0: f64,
    growth_rate: f64,
    ...
    // Optimizer states (kept for potential extensions/monitoring)
    exp_avg: HashMap<String, Tensor>,
    exp_avg_sq: HashMap<String, Tensor>,
    d_numerator: HashMap<String, Tensor>,
    d_denominator: HashMap<String, Tensor>,
...
impl Prodigy {
    /// Create a new Prodigy state container
    pub fn new(
        _vars: Vec<Parameter>,
        d0: f64,
        growth_rate: f64,
    ...
    /// Initialize optimizer states for a variable (kept for completeness)
    fn init_state(&mut self, var_id: String, shape: &[usize], device: &Device) -> anyhow::Result<()> {
        if !self.exp_avg.contains_key(&var_id) {
            let zeros = Tensor::zeros(flame_core::Shape::from_dims(shape), device.clone())?
                .to_dtype(DType::F32)?;
            self.exp_avg.insert(var_id.clone(), zeros.clone());
            self.exp_avg_sq.insert(var_id.clone(), zeros.clone());
            self.d_numerator.insert(var_id.clone(), zeros.clone());
    ...
    /// Update the global D estimate using accumulated stats
    fn update_d_estimate(&mut self) -> anyhow::Result<()> {
        let mut global_d_numerator = 0.0;
        let mut global_d_denominator = 0.0;

        for (k, d_num) in self.d_numerator.iter() {
            if let Some(d_den) = self.d_denominator.get(k) {
                let num_sum: f32 = d_num.flatten_all()?.to_vec1::<f32>()?.iter().sum();
                let den_sum: f32 = d_den.flatten_all()?.to_vec1::<f32>()?.iter().sum();
                global_d_numerator += num_sum as f64;
                global_d_denominator += den_sum as f64;
    ...
    /// Current effective learning rate estimate (D)
    pub fn current_learning_rate(&self) -> f64 { self.d_estimate }

    /// Current S estimate (reserved for future use/monitoring)
    pub fn current_s_estimate(&self) -> f64 { self.s_estimate }

    /// No-op step to keep API stable; training uses ProdigyOptimizer in optimizer.rs
    pub fn step_stub(&mut self, _params: &[&Tensor], _grads: &[Tensor]) -> anyhow::Result<()> {
        self.step += 1;
        Ok(())
    }
}
...
pub struct ProdigyConfig {
    pub d0: f64,            // Initial D estimate (default: 1e-6)
    pub growth_rate: f64,   // Growth rate for D (default: inf, meaning no upper bound)
    pub beta1: f64,         // EMA coefficient for gradient (default: 0.9)
    pub beta2: f64,         // EMA coefficient for squared gradient (default: 0.999)
    pub beta3: f64,         // EMA coefficient for D estimation (default: 0.999)
    pub epsilon: f64,       // Small constant for numerical stability (default: 1e-8)
    pub weight_decay: f64,  // Weight decay coefficient (default: 0.0)
}
...
/// Prodigy with different presets for common use cases
impl ProdigyConfig {
    pub fn for_language_models() -> Self { Self { weight_decay: 0.01, ..Default::default() } }
    pub fn for_vision() -> Self { Self { weight_decay: 0.05, ..Default::default() } }
    pub fn for_finetuning() -> Self { Self { d0: 1e-7, growth_rate: 10.0, weight_decay: 0.01, ..Default::default() } }
}
...
impl ProdigyMonitor {
    pub fn new() -> Self {
        Self { d_history: Vec::new(), s_history: Vec::new(), step_history: Vec::new() }
    }
