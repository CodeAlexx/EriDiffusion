...
pub trait Scheduler: Send + Sync {
    ...
    /// Set scheduler state
    fn set_state(&mut self, state: SchedulerState) -> anyhow::Result<()>;
}
...
impl SchedulerState {
    pub fn new(lr: f64) -> Self {
        Self { last_step: 0, last_lr: lr }
    }
}

impl Default for SchedulerState {
    fn default() -> Self {
        Self { last_step: 0, last_lr: 0.001 }
    }
}
...
/// Create scheduler
pub fn create_scheduler(config: SchedulerConfig) -> anyhow::Result<Box<dyn Scheduler>> {
...
impl ConstantScheduler {
    pub fn new(config: SchedulerConfig) -> Self {
        Self { state: SchedulerState::new(config.base_lr), config }
    }
}
...
impl LinearScheduler {
    pub fn new(config: SchedulerConfig) -> Self {
        Self { state: SchedulerState::new(0.0), config }
    }
}
...
impl Scheduler for PolynomialScheduler {
    fn step(&mut self, step: usize) -> f64 {
        ...
        let lr = if step < self.config.warmup_steps {
            ...
            // Polynomial decay
            let remaining = 1.0
                - (step - self.config.warmup_steps) as f64
                    / (total_steps - self.config.warmup_steps) as f64;
...
pub fn register_builtin_schedulers() -> anyhow::Result<()> {
