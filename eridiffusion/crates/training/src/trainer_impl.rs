...
impl DiffusionTrainer {
    ...
    pub fn new(
        ...
        let sampler = TrainingSampler::new(sampling_config, device);
    ...
    pub async fn resume_from_checkpoint(&mut self, checkpoint_path: PathBuf) -> anyhow::Result<()> {
    ...
    pub async fn train_epoch(
        ...
        while let Some(batch_result) = dataloader.next().await {
            ...
            let latent_batch = eridiffusion_data::batch_to_latents(
                batch,
                self.vae.as_ref(),
    ...
    async fn training_step(
        ...
        // Sample timesteps
        let batch_size = prepared_batch.latents.dim(0);
        ...
        // Optimizer step
        let state = self.state.read().await;
        optimizer.step(&params, &grads, state.learning_rate as f64)?; // TODO: Use gradient_map instead of individual tensor
        
        Ok(loss.item::<f32>()?)
    }
    
    /// Validation step with sampling
    async fn validation_step(&self, step: usize) -> anyhow::Result<()> {
