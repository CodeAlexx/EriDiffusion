...
mod tests {
    ...
    fn create_mock_text_encoder(hidden_size: usize, device: &Device) -> anyhow::Result<TextEncoder> {
        // This would be a mock implementation
        Ok(TextEncoder::mock(hidden_size, device)?)
    }
    
    #[test]
    fn test_encode_text_triple() -> anyhow::Result<()> {
        ...
        assert_eq!(result.encoder_hidden_states.shape().dims()[2]?, 8192);
        
        Ok(())
    }
    
    #[test]
    fn test_prepare_training_inputs() -> anyhow::Result<()> {
        let device = Device::cuda(0)?;
        let batch_size = 2;
        ...
        let pooled_projections = Tensor::randn(batch_size, 2048, DType::F32, device.clone())?;
    ...
    #[test]
    fn test_compute_flow_matching_loss() -> anyhow::Result<()> {
        let device = Device::cuda(0)?;
        let batch_size = 2;
    ...
    #[test]
    fn test_get_sd35_target_modules() -> anyhow::Result<()> {
    ...
    #[test]
    fn test_sd35_training_config() -> anyhow::Result<()> {
    ...
    #[test]
    fn test_velocity_prediction() -> anyhow::Result<()> {
        let device = Device::cuda(0)?;
        let batch_size = 2;
    ...
    #[test]
    fn test_snr_weighting() -> anyhow::Result<()> {
        let device = Device::cuda(0)?;
        let batch_size = 4;
    ...
    #[test]
    fn test_training_step_integration() -> anyhow::Result<()> {
        let device = Device::cuda(0)?;
        let batch_size = 2;
        ...
        let latents = Tensor::randn(batch_size, 16, 32, 32, DType::F32, &device)?;
        let text_embeddings = Tensor::randn(batch_size, 77, 8192, DType::F32, &device)?;
        let pooled = Tensor::randn(batch_size, 2048, DType::F32, device.clone())?;
