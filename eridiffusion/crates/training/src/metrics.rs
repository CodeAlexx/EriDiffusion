...
pub struct MetricsTracker {
    ...
    _custom_names: HashMap<u64, String>,
    max_history: usize,
}
...
impl MetricsTracker {
    ...
    pub fn with_max_history(max_history: usize) -> Self {
        Self { metrics: HashMap::new(), _custom_names: HashMap::new(), max_history }
    }
    ...
    pub fn log_at_step(&mut self, metric_name: &str, value: f32, step: usize) {
        let entry = MetricEntry { step, value, timestamp: Self::current_timestamp() };
    ...
    pub fn get_latest(&self, metric_name: &str) -> Option<f32> {
        self.metrics.get(metric_name).and_then(|history| history.last()).map(|entry| entry.value)
    }
    ...
    pub fn export_json(&self) -> Result<String> {
        serde_json::to_string_pretty(&self.metrics).map_err(|e| Error::Serialization(e.to_string()))
    }

    /// Import metrics from JSON
    pub fn import_json(&mut self, json: &str) -> Result<()> {
        let metrics: HashMap<String, Vec<MetricEntry>> =
...
impl MetricsAggregator {
    ...
    pub fn new(world_size: usize) -> Self {
        Self { local_metrics: MetricsTracker::new(), _world_size: world_size }
    }
    ...
    pub async fn aggregate(&self) -> anyhow::Result<HashMap<String, f32>> {
