...
impl GradientAccumulator {
    ...
    pub fn new(config: AccumulationConfig) -> Self {
        Self { config, accumulated_grads: HashMap::new(), current_step: 0 }
    }

    /// Accumulate gradients
    pub fn accumulate(&mut self, name: &str, gradient: &Tensor, scale: Option<f32>) -> Result<()> {
        let scale = scale.unwrap_or(1.0);

        if let Some(acc_grad) = self.accumulated_grads.get_mut(name) {
            // Add to existing accumulation
            let scaled =
                if scale != 1.0 { gradient.mul_scalar(scale as f32)? } else { gradient.clone() };

            acc_grad.gradient = acc_grad.gradient.add(&scaled)?;
            ...
            self.accumulated_grads.insert(
                name.to_string(),
                AccumulatedGradient { gradient, num_accumulations: 1, requires_sync: true },
            );
        }
    ...
    pub fn get_gradients(&mut self) -> Result<HashMap<String, Tensor>> {
        if !self.should_sync() {
            return Err(Error::InvalidInput("Accumulation not complete".into()));
        }
            ...
            let grad = match self.config.reduction {
                ReductionMode::Mean => {
                    if self.config.normalize {
                        acc_grad.gradient.div_scalar(acc_grad.num_accumulations as f32)?
                    } else {
                        acc_grad.gradient.div_scalar(self.config.steps as f32)?
                    }
                }
                ReductionMode::Sum => acc_grad.gradient.clone(),
                ReductionMode::WeightedMean(weight) => {
                    let divisor = if self.config.normalize {
                        acc_grad.num_accumulations as f32
                    } else {
                        self.config.steps as f32
                    };
                    acc_grad.gradient.affine(weight as f32 / divisor, 0.0f32)?
                }
            };
    ...
    pub fn memory_usage(&self) -> usize {
        self.accumulated_grads
            .values()
            .map(|acc| acc.gradient.shape().elem_count() * 4) // Assuming f32
            .sum()
    }
}
...
impl DynamicAccumulator {
    ...
    pub fn get_optimal_config(&self) -> AccumulationConfig {
        ...
        let avg_time: f32 =
            self.performance_monitor.step_times.iter().rev().take(10).sum::<f32>() / 10.0;

        let avg_memory: usize =
            self.performance_monitor.memory_usage.iter().rev().take(10).sum::<usize>() / 10;
        ...
        if avg_time < 0.75 && avg_memory < self.memory_threshold / 2 {
            config.steps = (config.steps + 1).min(64);
        }
...
impl GradientCheckpointing {
    pub fn new(enabled: bool, segment_size: usize) -> Self {
        Self { enabled, segment_size, checkpointed_layers: Vec::new() }
    }
...
pub mod utils {
    ...
    pub fn estimate_memory_requirement(
        ...
        let activation_memory = model_params * 4 * batch_size * accumulation_steps * dtype_bytes;
