...
use eridiffusion_core::{Error, NetworkAdapter};
...
impl CheckpointManager {
    ...
    pub fn new(
        ...
        Ok(Self { output_dir, save_latest, save_total_limit })
    }
    ...
    ) -> anyhow::Result<(CheckpointMetadata, HashMap<String, Vec<f32>>)> {
        // Load metadata
    ...
    fn load_safetensors(&self, path: &Path) -> anyhow::Result<HashMap<String, Vec<f32>>> {
        let data = fs::read(path)?;
        let tensors = SafeTensors::deserialize(&data)
            .map_err(|e| anyhow::anyhow!(format!("Failed to deserialize safetensors: {}", e)))?;
    ...
    fn cleanup_old_checkpoints(&self) -> anyhow::Result<()> {
...
fn serialize_tensors(
...
) -> eridiffusion_core::Result<Vec<u8>> {
    ...
    let metadata_opt = if metadata.is_empty() { None } else { Some(metadata.clone()) };

    Ok(serialize(&data, &metadata_opt)
