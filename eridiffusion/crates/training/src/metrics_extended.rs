...
pub trait MetricLogger: Send + Sync {
    async fn log_scalar(&self, name: &str, value: f32, step: usize) -> anyhow::Result<()>;
    async fn log_histogram(&self, name: &str, values: &[f32], step: usize) -> anyhow::Result<()>;
    async fn log_image(&self, name: &str, image: &[u8], step: usize) -> anyhow::Result<()>;
    async fn log_text(&self, name: &str, text: &str, step: usize) -> anyhow::Result<()>;
...
impl ConsoleLogger {
    pub fn new(interval: usize, format: ConsoleFormat) -> Self {
        Self { interval, last_log: Arc::new(RwLock::new(0)), format }
    }
}
...
impl MetricLogger for ConsoleLogger {
    ...
    async fn log_histogram(&self, name: &str, values: &[f32], step: usize) -> anyhow::Result<()> {
        ...
        println!(
            "[Step {}] {} histogram - mean: {:.4}, std: {:.4}, min: {:.4}, max: {:.4}",
            step, name, stats.mean, stats.std, stats.min, stats.max
        );
        Ok(())
    }

    async fn log_image(&self, name: &str, _image: &[u8], step: usize) -> anyhow::Result<()> {
...
pub struct TensorBoardLogger {
    _log_dir: String,
    _writer: Arc<RwLock<Option<()>>>, // Would be actual TensorBoard writer
}
...
impl TensorBoardLogger {
    pub async fn new(log_dir: &str) -> anyhow::Result<Self> {
        ...
        Ok(Self { _log_dir: log_dir.to_string(), _writer: Arc::new(RwLock::new(None)) })
    }
}

#[async_trait::async_trait]
impl MetricLogger for TensorBoardLogger {
    async fn log_scalar(&self, _name: &str, _value: f32, _step: usize) -> anyhow::Result<()> {
    ...
    async fn log_image(&self, _name: &str, _image: &[u8], _step: usize) -> anyhow::Result<()> {
        // Would write image to TensorBoard
        Ok(())
    }

    async fn log_text(&self, _name: &str, _text: &str, _step: usize) -> anyhow::Result<()> {
...
impl CSVLogger {
    ...
    async fn write_buffer(&self) -> anyhow::Result<()> {
...
impl MetricLogger for CSVLogger {
    ...
    async fn log_text(&self, name: &str, _text: &str, step: usize) -> anyhow::Result<()> {
...
fn calculate_statistics(values: &[f32]) -> Statistics {
    if values.is_empty() {
        return Statistics { mean: 0.0, std: 0.0, min: 0.0, max: 0.0, median: 0.0 };
    }
