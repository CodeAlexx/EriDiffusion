...
impl ProcessGroup {
    ...
    pub async fn init(config: DistributedConfig) -> Result<Self> {
        ...
        Ok(Self { config, communicator: Arc::new(RwLock::new(communicator)), barrier })
    }
...
impl NcclCommunicator {
    async fn new(config: &DistributedConfig) -> Result<Self> {
        ...
        Ok(Self { rank: config.rank, world_size: config.world_size })
    }
}
...
impl Communicator for NcclCommunicator {
    ...
    async fn broadcast(&self, _tensor: &mut Tensor, _src: usize) -> Result<()> {
    ...
    async fn reduce_scatter(&self, tensors: &[Tensor], _op: ReduceOp) -> Result<Tensor> {
...
impl Communicator for GlooCommunicator {
    async fn all_reduce(&self, _tensor: &mut Tensor, _op: ReduceOp) -> Result<()> {
        Ok(())
    }
...
impl<M> DistributedDataParallel<M> {
    ...
    pub fn new(module: M, process_group: Arc<ProcessGroup>) -> anyhow::Result<Self> {
        let (broadcast_tx, _) = broadcast::channel(16);

        Ok(Self {
            _module: Arc::new(RwLock::new(module)),
            process_group,
    ...
    pub async fn sync_gradients(&self) -> anyhow::Result<()> {
        ...
        for bucket in buckets.iter_mut() {
            if bucket.pending {
                ...
                let flat = if bucket.tensors.len() == 1 {
                    ...
                    let flattened: Vec<Tensor> = bucket
                        .tensors
                        .iter()
    ...
    pub async fn broadcast_parameters(&self) -> anyhow::Result<()> {
...
pub mod utils {
    ...
    /// Get device for rank
    pub fn get_device_for_rank(_rank: usize, local_rank: usize) -> Device {
        Device::Cuda(local_rank)
    }

    /// Initialize from environment
    pub fn init_from_env() -> anyhow::Result<DistributedConfig> {
        use std::env;
