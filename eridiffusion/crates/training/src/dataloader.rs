...
use eridiffusion_core::{DType, Device, Error, ModelInputs, Result};
use eridiffusion_models::devtensor::{shape1, shape3, tensor_from_vec_on, zeros_on};
...
impl DataLoaderIterator {
    fn new(loader: &DataLoader) -> Self {
        ...
        Self { loader, current_batch: 0, total_batches }
    }
    ...
    async fn get_batch(&self, batch_idx: usize) -> Result<ModelInputs> {
        ...
        for idx in batch_indices {
            ...
            let handle = tokio::spawn(async move { dataset.get_item(idx).await });
            handles.push(handle);
        }
    ...
    async fn collate_batch(&self, items: Vec<DatasetItem>) -> Result<ModelInputs> {
        if items.is_empty() {
            return Err(Error::DataError("Empty batch".into()));
        }

        // Stack images as latents placeholder (NHWC expected downstream)
        let images: Vec<Tensor> = items.iter().map(|it| it.image.clone()).collect();
        let latents = Tensor::stack(&images, 0)?;

        // Timesteps tensor [B] zeros
        let b = items.len();
        let timestep =
            zeros_on(shape1(b as i64), &self.loader.device, DType::F32).map_err(Error::from)?;

        // Placeholder encoder hidden states [B,77,768] zeros
        let encoder_hidden_states =
            zeros_on(shape3(b as i64, 77, 768), &self.loader.device, DType::F32)
                .map_err(Error::from)?;

        let additional = std::collections::HashMap::new();
...
impl ImageFolderDataset {
    ...
    pub async fn new(
        ...
        let mut entries = tokio::fs::read_dir(root_path).await.map_err(|e| Error::Io(e))?;
...
impl Dataset for ImageFolderDataset {
    ...
    async fn get_item(&self, index: usize) -> Result<DatasetItem> {
        ...
        let image_bytes = std::fs::read(image_path).map_err(|e| {
            Error::DataError(format!("Failed to read image {}: {}", image_path.display(), e))
        })?;
        ...
        let raw_data: Vec<f32> =
            img.pixels().flat_map(|p| p.0.iter().map(|&v| v as f32 / 255.0)).collect();

        let dev = eridiffusion_core::Device::cuda_if_available(0)?;
        let image =
            tensor_from_vec_on(raw_data, shape3(height as i64, width as i64, 3), &dev, DType::F32)
                .map_err(Error::from)?;

        // Apply transform if available
        let image =
            if let Some(ref transform) = self.transform { transform.apply(&image)? } else { image };

        let caption = self.captions[index].clone();

        Ok(DatasetItem { image, caption, additional: std::collections::HashMap::new() })
    }
