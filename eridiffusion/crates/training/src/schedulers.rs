...
impl LinearLR {
    pub fn new(base_lr: f32, total_steps: usize, config: SchedulerConfig) -> Self {
        Self { config, base_lr, total_steps, last_lr: base_lr }
    }
}
...
impl CosineAnnealingWarmRestarts {
    pub fn new(base_lr: f32, t_0: usize, t_mult: f32, config: SchedulerConfig) -> Self {
        Self { config, base_lr, t_0, t_mult, last_lr: base_lr, current_cycle: 0, cycle_step: 0 }
    }
}
...
impl OneCycleLR {
    pub fn new(max_lr: f32, total_steps: usize, pct_start: f32, config: SchedulerConfig) -> Self {
        Self {
            config,
            max_lr,
            total_steps,
...
impl LRScheduler for OneCycleLR {
    fn get_lr(&self, step: usize) -> f32 {
        ...
        if step < start_step {
            ...
            // Decreasing phase
            let progress = (step - start_step) as f32 / (self.total_steps - start_step) as f32;
            ...
            match self.anneal_strategy.as_str() {
                ...
                "linear" => self.max_lr * (1.0 - progress).max(0.0) + self.config.min_lr,
                _ => self.max_lr,
            }
        }
    }
...
impl ReduceLROnPlateau {
    pub fn new(base_lr: f32, factor: f32, patience: usize, config: SchedulerConfig) -> Self {
...
pub fn create_scheduler(
    ...
    match config.scheduler_type {
        SchedulerType::Constant => Box::new(ConstantLR::new(base_lr, config)),
        SchedulerType::Linear => Box::new(LinearLR::new(base_lr, total_steps, config)),
        SchedulerType::Cosine => Box::new(CosineAnnealingLR::new(base_lr, total_steps, config)),
...
pub mod utils {
    ...
    pub fn plot_schedule(scheduler: &dyn LRScheduler, total_steps: usize) -> Vec<f32> {
        (0..total_steps).map(|step| scheduler.get_lr(step)).collect()
    }

    /// Find optimal warmup steps
    pub fn find_optimal_warmup(_base_lr: f32, total_steps: usize) -> usize {
