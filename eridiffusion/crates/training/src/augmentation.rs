...
impl Default for ColorJitterConfig {
    fn default() -> Self {
        Self { brightness: 0.2, contrast: 0.2, saturation: 0.2, hue: 0.1 }
    }
}
...
impl AugmentationPipeline {
    ...
    pub fn new(config: AugmentationConfig) -> Self {
        Self { config, rng: rand::thread_rng() }
    }
    ...
    pub fn mixup(
        ...
        if let Some(alpha) = self.config.mixup {
            ...
            // Mix images
            let img1_scaled = image1.affine(lambda as f32, 0.0f32)?;
            let img2_scaled = image2.affine((1.0 - lambda) as f32, 0.0f32)?;
            let mixed_image = img1_scaled.add(&img2_scaled)?;

            // Mix labels
            let label1_scaled = label1.affine(lambda as f32, 0.0f32)?;
            let label2_scaled = label2.affine((1.0 - lambda) as f32, 0.0f32)?;
    ...
    fn random_flip(&self, image: &Tensor) -> Result<Tensor> {
        // Placeholder: return original image (flip implementation requires index_select setup)
        Ok(image.clone())
    }

    /// Rotate image
    fn rotate(&self, image: &Tensor, _angle: f32) -> Result<Tensor> {
    ...
    fn color_jitter(&mut self, image: &Tensor, config: &ColorJitterConfig) -> Result<Tensor> {
        ...
        if config.brightness > 0.0 {
            ...
            result = result.affine(factor as f32, 0.0f32)?;
        }

        // Contrast adjustment
        if config.contrast > 0.0 {
            let factor = 1.0 + self.rng.gen_range(-config.contrast..config.contrast);
            let mean = result.sum()?.div_scalar(result.shape().elem_count() as f32)?;
            let mean_val = mean.item()?;
            result = result
                .affine(1.0f32, -mean_val)?
                .affine(factor as f32, 0.0f32)?
                .affine(1.0f32, mean_val)?;
        }

        // Clamp values
        result = result.clamp(0.0f32, 1.0f32)?;
    ...
    fn random_crop(&mut self, image: &Tensor, crop_size: (usize, usize)) -> Result<Tensor> {
        let dims = image.dims();
        if dims.len() < 3 {
            return Err(Error::Model("Image must have at least 3 dimensions".into()));
        }
        ...
        // Crop image
        image
            .narrow(dims.len() - 2, y, crop_size.0)?
            .narrow(dims.len() - 1, x, crop_size.1)
            .map_err(|e| Error::Tensor(e.to_string()))
    }

    /// Random resize
    fn random_resize(&self, image: &Tensor, _scale: f32) -> Result<Tensor> {
    ...
    fn cutout(&mut self, image: &Tensor, config: &CutoutConfig) -> Result<Tensor> {
        // Minimal no-op cutout placeholder to keep API surface stable
        let _ = config;
        Ok(image.clone())
    }
    ...
    fn sample_beta(&mut self, alpha: f32, beta: f32) -> f64 {
        // Beta distribution sampling using the rejection method
...
impl CaptionAugmentation {
    ...
    pub fn new(dropout_prob: f32, shuffle_prob: f32, synonym_prob: f32) -> Self {
        Self { dropout_prob, shuffle_prob, synonym_prob }
    }

    /// Get synonym for a word
    fn get_synonym(&self, word: &str) -> String {
        // Basic synonym dictionary for common words in image captions
        let synonyms: std::collections::HashMap<&str, Vec<&str>> = [
            ("beautiful", vec!["gorgeous", "stunning", "lovely", "attractive"]),
            ("big", vec!["large", "huge", "enormous", "massive"]),
            ("small", vec!["tiny", "little", "miniature", "petite"]),
            ("happy", vec!["joyful", "cheerful", "delighted", "pleased"]),
            ("sad", vec!["unhappy", "melancholy", "sorrowful", "gloomy"]),
            ("fast", vec!["quick", "rapid", "swift", "speedy"]),
            ("slow", vec!["sluggish", "leisurely", "gradual", "unhurried"]),
            ("bright", vec!["luminous", "radiant", "brilliant", "vivid"]),
            ("dark", vec!["dim", "shadowy", "gloomy", "murky"]),
            ("old", vec!["ancient", "aged", "elderly", "vintage"]),
            ("new", vec!["fresh", "modern", "recent", "novel"]),
            ("good", vec!["excellent", "fine", "great", "superb"]),
            ("bad", vec!["poor", "terrible", "awful", "inferior"]),
        ]
        .iter()
        .cloned()
        .collect();
        ...
        if let Some(synonym_list) = synonyms.get(word.to_lowercase().as_str()) {
            let mut rng = rand::thread_rng();
            let idx = rng.gen_range(0..synonym_list.len());
            synonym_list[idx].to_string()
        } else {
    ...
    pub fn apply(&self, caption: &str) -> String {
        ...
        // Synonym replacement with basic synonym mapping
...
impl LatentAugmentation {
    ...
    pub fn new(noise_scale: f32, flip_prob: f32) -> Self {
        Self { noise_scale, flip_prob }
    }

    /// Apply augmentation to latents
    pub fn apply(&self, latents: &Tensor) -> anyhow::Result<Tensor> {
        let mut rng = rand::thread_rng();
        let mut result = latents.clone();

        // Add noise
        if self.noise_scale > 0.0 {
            let noise = Tensor::rand_like(latents)?.mul_scalar(self.noise_scale)?;
            result = result.add(&noise)?;
        }

        // Random flip
        if rng.gen_bool(self.flip_prob as f64) {
            // Placeholder: skip flip to avoid unsupported gather; keep API stable
        }
