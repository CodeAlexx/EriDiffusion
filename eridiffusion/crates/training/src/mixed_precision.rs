...
pub enum LossScaleConfig {
    Static(f32),
    Dynamic { init_scale: f32, growth_factor: f32, backoff_factor: f32, growth_interval: usize },
}
...
impl GradScaler {
    ...
    pub async fn scale(&self, loss: &Tensor) -> Result<Tensor> {
        ...
        let scale = *self.current_scale.read().await as f32;
        Ok(loss.mul_scalar(scale)?)
    }

    /// Check gradients for Inf/NaN in FP32 view (synchronous helper)
    pub fn found_inf(&self, grads: &[Tensor]) -> Result<bool> {
        for g in grads {
            let g32 = if g.dtype() != DType::F32 { g.to_dtype(DType::F32)? } else { g.clone() };
            if self.has_inf_nan(&g32)? {
                return Ok(true);
            }
        }
        Ok(false)
    ...
    pub async fn update_scale(&self) -> Result<()> {
        ...
        match &self.config.loss_scale {
            ...
            LossScaleConfig::Dynamic { growth_factor, backoff_factor, growth_interval, .. } => {
                if found_inf {
                    // Decrease scale
    ...
    /// Set current scale (for resume)
    pub async fn set_scale(&self, scale: f32) {
        *self.current_scale.write().await = scale;
    }

    /// Get internal growth tracker (for resume)
    pub async fn get_growth_tracker(&self) -> usize {
        *self.growth_tracker.read().await
    }

    /// Set internal growth tracker (for resume)
    pub async fn set_growth_tracker(&self, v: usize) {
        *self.growth_tracker.write().await = v;
    }

    /// Check if tensor has inf/nan
    fn has_inf_nan(&self, tensor: &Tensor) -> Result<bool> {
        let flat = tensor.flatten_all()?;
        let max_val = flat.max_all()?;
        let min_val = flat.min_all()?;

        Ok(max_val.is_infinite() || max_val.is_nan() || min_val.is_infinite() || min_val.is_nan())
    }
    ...
    pub fn unscale(&self, grads: &Vec<Tensor>) -> Result<Vec<Tensor>> {
        ...
        let scale =
            tokio::runtime::Handle::current().block_on(async { *self.current_scale.read().await });

        grads
            .iter()
            .map(|g| {
                g.affine(1.0f32 / scale as f32, 0.0f32).map_err(|e| Error::Training(e.to_string()))
            })
...
impl<O> MixedPrecisionOptimizer<O> {
    pub fn new(optimizer: O, scaler: GradScaler, model_params: Vec<Tensor>) -> Result<Self> {
    ...
    pub async fn step(&mut self, gradients: &[Tensor]) -> Result<bool> {
        ...
        for (master, grad) in master_params.iter_mut().zip(&unscaled_grads) {
            ...
            *master = master.sub(&grad.mul_scalar(0.001f32)?)?; // Simplified
        }

        // Copy back to model parameters
        let model_params = self.model_params.read().await;
        for (_, _master) in model_params.iter().zip(master_params.iter()) {
            // Convert back to model dtype
            let _dtype = match self.scaler.config.dtype {
...
fn clip_grad_norm(gradients: &mut [Tensor], max_norm: f32) -> Result<()> {
    ...
    for grad in gradients.iter() {
        let l2_sq = grad.flatten_all()?.square()?.sum()?.item()?;
        total_norm += l2_sq;
    }
...
impl AMPContext {
    pub fn new(config: &MixedPrecisionConfig, device: Device) -> Self {
        ...
        let whitelist = vec!["matmul".to_string(), "conv2d".to_string(), "linear".to_string()];
        ...
        Self { enabled: config.enabled, dtype, _device: device, whitelist, blacklist }
    }
    ...
    pub fn autocast(&self, tensor: &Tensor, op: &str) -> Result<Tensor> {
        ...
        if self.whitelist.contains(&op.to_string()) {
            ...
            tensor.to_dtype(self.dtype).map_err(eridiffusion_core::Error::from)
        } else if self.blacklist.contains(&op.to_string()) {
            // Keep in FP32
            tensor.to_dtype(DType::F32).map_err(eridiffusion_core::Error::from)
        } else {
    ...
    pub fn estimate_memory_savings(
        ...
        MemoryEstimate {
            ...
            savings_percent: ((fp32_memory + fp32_activation - mp_memory - mp_activation) as f32
                / (fp32_memory + fp32_activation) as f32
                * 100.0),
        }
    }
}
