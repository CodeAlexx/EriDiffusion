...
impl FluxDoubleBlockWithLoRA {
    ...
    fn forward_img_stream(
        ...
        let img = img.add(&(gate1.unsqueeze(1)?.mul(&img_attn_out)?))?;
        ...
        // Gated residual connection
        img.add(&(gate2.unsqueeze(1)?.mul(&mlp_out)?))
    }
    ...
    fn forward_txt_stream(
        ...
        // Extract text part
        let txt_seq_len = txt.shape().dim(1)?;
        let txt_attn_out = attn_out.narrow(1, 0, txt_seq_len)?;
        
        // Gated residual connection
        let txt = txt.add(&(gate1.unsqueeze(1)?.mul(&txt_attn_out)?))?;
        ...
        // Gated residual connection
        txt.add(&(gate2.unsqueeze(1)?.mul(&mlp_out)?))
    }
}
...
impl FluxSingleBlockWithLoRA {
    ...
    pub fn forward(&self, x: &Tensor, vec: &Tensor) -> anyhow::Result<Tensor> {
        // Self-attention with modulation
        ...
        // Gated residual
        let x = x.add(&(gate1.unsqueeze(1)?.mul(&attn_out)?))?;
        ...
        // Gated residual
        x.add(&(gate2.unsqueeze(1)?.mul(&mlp_out)?))
    }
}
...
use flame_core::Shape;
    ...
    #[test]
    fn test_double_block_forward() -> anyhow::Result<()> {
        ...
        // Test inputs
        let img = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, img_seq_len, hidden_size]), &device)?;
        let txt = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, txt_seq_len, hidden_size]), &device)?;
