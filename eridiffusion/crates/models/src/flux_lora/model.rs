...
impl FluxTimeEmbedding {
    fn new(hidden_size: usize, vb: WeightLoader) -> anyhow::Result<Self> {
        Ok(Self {
...
impl flame_core::Module for FluxTimeEmbedding {
...
impl FluxModelWithLoRA {
    ...
    pub fn new(
        ...
        // Output layers
        let final_norm = 1e-6, vb.pp("final_norm"))?;
    ...
    pub fn trainable_parameters(&self) -> Vec<&flame_core::Parameter> {
    ...
    pub fn forward(&self, inputs: &ModelInputs) -> anyhow::Result<ModelOutput> {
        let latents = &inputs.latents;
        let timestep = &inputs.timestep;
        let encoder_hidden_states = inputs
            .encoder_hidden_states
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("Flux requires text embeddings"))?;
        ...
        if let Some(guidance_scale) = inputs.guidance_scale {
            if let Some(guidance_in) = &self.guidance_in {
                let guidance_emb = Tensor::from_vec(&[guidance_scale], vec.device())?
                    .unsqueeze(0)?
                    .repeat((vec.shape().dim(0)?, 1))?;
        ...
        let img_seq_len = img.shape().dim(1)?;
    ...
    fn embed_image(&self, x: &Tensor) -> anyhow::Result<Tensor> {
        ...
        let positions = Tensor::arange(0, x.shape().dim(1)? as i64, x.device())?;
        let pos_emb = self.pos_embed.forward(&positions)?;
        x.add(&pos_emb)?
    }
    
    /// Unpatchify output back to image format
    fn unpatchify(&self, x: &Tensor) -> anyhow::Result<Tensor> {
...
fn timestep_embedding(timesteps: &Tensor, dim: usize) -> anyhow::Result<Tensor> {
    let half_dim = dim / 2;
...
impl DiffusionModel for FluxModelWithLoRA {
    fn forward(&self, inputs: &ModelInputs) -> anyhow::Result<ModelOutput> {
        self.forward(inputs)
    }
...
mod tests {
    ...
    #[test]
    fn test_flux_model_creation() -> anyhow::Result<()> {
