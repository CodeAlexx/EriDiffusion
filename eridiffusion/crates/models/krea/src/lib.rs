...
pub struct KreaModel { pub dim: usize, pub n_layers: usize, pub n_heads: usize, pub ctx_dim: usize }
impl KreaModel { pub fn new() -> Self { Self { dim: 3072, n_layers: 24, n_heads: 24, ctx_dim: 2048 } } }

impl DiffusionModule for KreaModel {
...
mod register {
    ...
    struct Wrap(super::KreaModel);
    impl regy::DiffusionModule for Wrap {
        fn forward(&self, latents: &Tensor, t: &Tensor, ctx: Option<&Tensor>) -> Result<Tensor> { super::KreaModel::forward(&self.0, latents, t, ctx) }
    }
    fn build(_cfg: &serde_yaml::Value, _load: &regy::LoadSpec, _reg: &mut ParamRegistry, _device: &flame_core::Device) -> Result<Box<dyn regy::DiffusionModule>> {
        Ok(Box::new(Wrap(KreaModel::new())))
    }
    inventory::submit! { regy::ModelEntry { id: "krea", build } }
}
