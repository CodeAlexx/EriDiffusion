...
impl ConditioningEmbedding {
    fn new(
        ...
        // Input convolution
        let conv_in = block_out_channels[0],
            3,
            conv_config,
            WeightLoader::zeros(DType::F32, device),
        )?;
        ...
        // Output convolution
        let conv_out = conditioning_embedding_channels,
            3,
            conv_config,
            WeightLoader::zeros(DType::F32, device),
        )?;
    ...
    fn forward(&self, conditioning: &Tensor) -> anyhow::Result<Tensor> {
        let mut x = self.conv_in.forward(conditioning)?;
        x = flame_core::nn::ops::silu(&x)?;
...
impl GroupNorm {
    fn new(
        ...
        Ok(Self {
            ...
            num_channels,
            eps,
            weight: Tensor::ones(Shape::from(vec![num_channels]), DType::F32, device)?,
            bias: Tensor::zeros(Shape::from(vec![num_channels]), DType::F32, device)?,
        })
    }
    
    fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
        let shape = x.dims();
        let batch_size = shape.dim(0);
        let num_channels = shape.dim(1);
        ...
        let mean = x.mean_keepdim(2isize as isize)?.mean_keepdim(3isize as isize)?.mean_keepdim(4isize as isize)?;
        let variance = (&x - &mean)?.sqr()?.mean_keepdim(2isize as isize)?.mean_keepdim(3isize as isize)?.mean_keepdim(4isize as isize)?;
...
impl ControlNetBlock {
    fn new(
        ...
        let conv1 = out_channels,
            3,
            conv_config,
            WeightLoader::zeros(DType::F32, device),
        )?;
        
        let conv2 = out_channels,
            3,
            conv_config,
            WeightLoader::zeros(DType::F32, device),
        )?;
        ...
        let time_emb_proj = time_embed_dim.map(|dim| {
            Linear::new(
                Tensor::randn(0.0f32, 0.02, Shape::from(vec![out_channels, dim]), device).unwrap(),
                None,
            )
        });
    ...
    fn forward(&self, x: &Tensor, time_emb: Option<&Tensor>) -> anyhow::Result<Tensor> {
        ...
        h = Some(ref proj)) = (time_emb, &self.time_emb_proj) {
            let temb = flame_core::nn::ops::silu(temb)?;
            let temb = proj.forward(&temb)?;
            h = (h + temb.unsqueeze(2)?.unsqueeze(3)?)?;
        }
        
        h = self.norm2.forward(&h)?;
        h = flame_core::nn::ops::silu(&h)?;
...
impl ControlNetAdapter {
    ...
    pub fn new(config: ControlNetConfig, device: Device) -> anyhow::Result<Self> {
        let config.conditioning_embedding_channels,
        ...
        let middle_channels = config.block_out_channels.last()
            .ok_or_else(|| Error::Network("block_out_channels cannot be empty".into()))?
            .clone();
        ...
        for &channels in &config.block_out_channels {
            let zero_conv = channels,
                ...
                WeightLoader::zeros(DType::F32, &)?;
            controlnet_down_blocks.push(zero_conv);
        }
        
        let controlnet_mid_block = middle_channels,
            1,
            Conv2dConfig {
                padding: 0,
                stride: 1,
    ...
    pub fn process_conditioning(&self, conditioning: &Tensor) -> anyhow::Result<Vec<BlockOutput>> {
...
impl NetworkAdapter for ControlNetAdapter {
    fn forward(&self, x: &Tensor, inputs: &eridiffusion_core::ModelInputs) -> anyhow::Result<NetworkOutput> {
        ...
        let conditioning = inputs.additional.get("controlnet_cond")
            .ok_or_else(|| Error::Model("Missing ControlNet conditioning".into()))?;
    ...
    fn merge_adapters(&mut self, _adapters: Vec<Box<dyn NetworkAdapter>>, _weights: Vec<f32>) -> anyhow::Result<()> {
        ...
        Err(Error::Model("ControlNet does not support adapter merging".into()))
    }
}
*/
...
pub mod utils {
    ...
    fn preprocess_canny(image: &Tensor) -> anyhow::Result<Tensor> {
    ...
    fn preprocess_depth(image: &Tensor) -> anyhow::Result<Tensor> {
        // Normalize depth map
        let min = image.min_keepdim(2isize)?.min_keepdim(3isize)?;
        let max = image.max_keepdim(2isize)?.max_keepdim(3isize)?;
        let normalized = ((image - &min)? / ((&max - &min)? + 1e-6)?)?;
        Ok(normalized)
    }
    
    fn preprocess_normal(image: &Tensor) -> anyhow::Result<Tensor> {
        // Normalize normal map to [-1, 1]
        Ok(((image * 2.0)? - 1.0)?)
    }
    
    fn preprocess_openpose(image: &Tensor) -> anyhow::Result<Tensor> {
        // OpenPose keypoints are already preprocessed
        Ok(image.clone())
    }
    
    fn preprocess_scribble(image: &Tensor) -> anyhow::Result<Tensor> {
        // Binarize scribble
        let threshold = 0.5;
