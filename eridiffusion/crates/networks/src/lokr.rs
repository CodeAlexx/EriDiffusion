...
impl LoKrLayer {
    ...
    pub fn new(
        ...
        let n1, m2, n2) = Self::compute_factorization(
            in_features,
            out_features,
            factor,
        )?;
        ...
        let lora_a1 = Linear::new(
            Self::kaiming_uniform_init(rank, n1, dtype, &None,
        );
        
        let lora_a2 = Linear::new(
            Self::kaiming_uniform_init(rank, n2, dtype, &None,
        );
        
        let lora_b1 = Linear::new(
            Tensor::zeros(Shape::from(vec![m1, rank]), dtype, &None,
        );
        
        let lora_b2 = Linear::new(
            Tensor::zeros(Shape::from(vec![m2, rank]), dtype, &None,
        );
        
        // Optional learnable scalars
        let (scalar_a, scalar_b) = if use_scalar {
            (
                Some(Tensor::ones(Shape::from(vec![rank]), dtype, &Some(Tensor::ones(Shape::from(vec![rank]), dtype, &)
        } else {
            (None, None)
        };
    ...
    pub fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
        ...
        let batch_dims = result.dims()[..result.rank()-1].to_vec();
        ...
        if let Some(ref scalar_b) = self.scalar_b {
            result = result.mul(&scalar_b.unsqueeze(0)?.unsqueeze(0)?)?;
        }
        
        // result @ B2ᵀ
        result = result.matmul(&b2_weight.transpose(0, 1)?)?; // [batch, m1, m2]
        ...
        result = result.reshape(output_shape.dims())?;
    ...
    pub fn get_weight(&self) -> anyhow::Result<Tensor> {
        ...
        let weight = b_scaled.matmul(&a_scaled.transpose(0, 1)?)?;
    ...
    fn kronecker_product(a: &Tensor, b: &Tensor) -> anyhow::Result<Tensor> {
        let (m1, n1) = (a.dim(0), a.dim(1));
        let (m2, n2) = (b.dim(0), b.dim(1));
        ...
        for i1 in 0..m1 {
            for i2 in 0..m2 {
                for j1 in 0..n1 {
                    for j2 in 0..n2 {
                        let a_val = a.get(i1 * n1 + j1)?.item::<f32>()?;
                        let b_val = b.get(i2 * n2 + j2)?.item::<f32>()?;
        ...
        Tensor::from_vec(result, Shape::from(vec![m1 * m2, n1 * n2]), a.device(),
...
impl LoKrAdapter {
    ...
    pub fn initialize_layers(
        ...
        for (name, info) in model_state_dict {
            if self.should_adapt_module(name) && info.shape.rank() >= 2 {
    ...
    pub fn num_parameters(&self) -> usize {
        ...
        state.layers.values().map(|layer| {
            ...
            if layer.scalar_a.is_some() {
                total += layer.lora_a1.weight().dim(0); // rank
            }
            
            if layer.scalar_b.is_some() {
                total += layer.lora_b1.weight().dim(1); // rank
            }
...
impl NetworkAdapter for LoKrAdapter {
    ...
    async fn save_weights(&self, path: &std::path::Path) -> anyhow::Result<()> {
    ...
    async fn load_weights(&mut self, path: &std::path::Path) -> anyhow::Result<()> {
                
        // Load tensors
        let device = self.state.read().device.clone();
        let _tensors = flame_core::safetensors::load(path, &device)?;
...
pub mod utils {
    ...
    pub fn analyze_kronecker_approximation(
    ...
    ) -> anyhow::Result<ApproximationMetrics> {
        let reconstructed = lokr_layer.get_weight()?;
        
        // Compute reconstruction error
        let error = (original_weight - &reconstructed)?.sqr()?.mean_all()?;
        let original_norm = original_weight.sqr()?.sum()?.sqrt()?;
        let relative_error = error.item::<f32>()? / original_norm.item::<f32>()?;
        ...
        Ok(ApproximationMetrics {
            reconstruction_error: error.item::<f32>()?,
            relative_error,
            compression_ratio,
            effective_rank: lokr_layer.lora_a1.weight().dim(0),
        })
    }
    ...
    pub fn lora_to_lokr(
        ...
        // Compute LoRA weight
        let weight = lora_b.matmul(&lora_a.transpose(0, 1)?)?;
        ...
        let a1 = Tensor::randn(0.0f32, 0.02, Shape::from(vec![lora_a.dim(0)), n1], weight.device())?;
        let a2 = Tensor::randn(0.0f32, 0.02, Shape::from(vec![lora_a.dim(0)), n2], weight.device())?;
        let b1 = Tensor::randn(0.0f32, 0.02, &[m1, lora_b.dim(0)], weight.device())?;
        let b2 = Tensor::randn(0.0f32, 0.02, &[m2, lora_b.dim(0)], weight.device())?;
...
impl LoKrLayer {
    fn num_parameters(&self) -> usize {
        ...
        if self.scalar_a.is_some() {
            total += self.lora_a1.weight().dim(0);
        }
        
        if self.scalar_b.is_some() {
            total += self.lora_b1.weight().dim(1);
        }
