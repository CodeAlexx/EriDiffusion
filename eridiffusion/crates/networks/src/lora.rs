...
impl LoRALayer {
    ...
    pub fn new(
        ...
        let in_features, dtype, &if use_bias {
        ...
        let lora_b = Linear::new(
            Tensor::zeros(Shape::from(vec![out_features, rank]), dtype, &if use_bias {
    ...
    pub fn merge(&mut self, base_weight: &Tensor) -> anyhow::Result<Tensor> {
        if self.merged {
    ...
    pub fn unmerge(&mut self, merged_weight: &Tensor) -> anyhow::Result<Tensor> {
        if !self.merged {
...
impl LoRAAdapter {
    ...
    pub fn initialize_layers(&self, model_state_dict: &HashMap<String, TensorInfo>) -> anyhow::Result<()> {
    ...
    pub fn apply_with_base(&self, layer_name: &str, x: &Tensor, base_output: &Tensor) -> anyhow::Result<Tensor> {
    ...
    pub fn merge_weights(&self, model_state_dict: &mut HashMap<String, Tensor>) -> anyhow::Result<()> {
        let mut state = self.state.write();
    ...
    pub fn unmerge_weights(&self, model_state_dict: &mut HashMap<String, Tensor>) -> anyhow::Result<()> {
        let mut state = self.state.write();
...
impl NetworkAdapter for LoRAAdapter {
    ...
    fn trainable_parameters(&self) -> Vec<&Parameter> {
        // TODO: Return actual Parameter references from LoRA layers
        // For now return empty as layers use Linear which doesn't expose Parameter directly
        Vec::new()
    }
    ...
    async fn load_weights(&mut self, path: &Path) -> anyhow::Result<()> {
        ...
        // Load tensors
        let device = self.state.read().device;
            ...
            for layer_name in layer_keys {
                ...
                if let (Some(a_weight), Some(b_weight)) = (tensors.get(&a_key), tensors.get(&b_key)) {
                    ...
                    let layer = LoRALayer::new(
                        a_shape.dim(1),  // in_features
                        b_shape.dim(0), // out_features
                        a_shape.dim(0),   // rank
...
pub mod utils {
    ...
    pub async fn svd_init_lora(
        ...
        let u = Tensor::randn(0.0f32, 0.02, Shape::from(vec![shape.dim(0)), rank], device)?;
        let v = Tensor::randn(0.0f32, 0.02, &[rank, shape.dim(1)], device)?;
...
mod tests {
    ...
    #[test]
    fn test_lora_layer() -> anyhow::Result<()> {
