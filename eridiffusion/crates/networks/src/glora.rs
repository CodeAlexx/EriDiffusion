...
pub struct GLoRAConv2d {
    // Core LoRA components
    lora_a: lora_b: // GLoRA-specific components
    layer_norm: Option<LayerNorm>,
    prompt_embeddings: Option<Tensor>,
    scaling_vector: Option<Tensor>,
    gating: Option<Linear>,
    ...
    // Multi-head variant
    head_lora_a: Option<Vec<head_lora_b: Option<Vec<scaling: f32,
    dropout: Option<f32>,
    rank: usize,
    in_channels: usize,
    out_channels: usize,
...
impl GLoRALinear {
    ...
    pub fn new(
        ...
        let in_features, dtype, &if config.use_bias {
                Some(Tensor::zeros(Shape::from(vec![rank]), dtype, &candle_device)?)
            } else {
                None
            },
        );
        ...
        let lora_b = Linear::new(
            Tensor::zeros(Shape::from(vec![out_features, rank]), dtype, &if config.use_bias {
                Some(Tensor::zeros(Shape::from(vec![out_features]), dtype, &candle_device)?)
            } else {
                None
            },
        );
        ...
        let layer_norm = if config.use_layernorm {
            Some(LayerNorm::new(
                Tensor::ones(Shape::from(vec![rank]), dtype, &Tensor::zeros(Shape::from(vec![rank]), dtype, &1e-5,
            ))
        } else {
            None
        };
        ...
        let prompt_embeddings = if let Some(prompt_len) = config.prompt_length {
            Some(Tensor::randn(0.0f32, 0.02, Shape::from(vec![prompt_len, rank]), &)?)
        } else {
            None
        };
        ...
        let scaling_vector = if config.use_scalar {
            Some(Tensor::ones(Shape::from(vec![rank]), dtype, &candle_device)?)
        } else {
            None
        };
        ...
        let gating = if config.hierarchical {
            Some(Linear::new(
                Tensor::randn(0.0f32, 0.02, Shape::from(vec![1, in_features]), &Some(Tensor::zeros(Shape::from(vec![1]), dtype, &))
        } else {
            None
        };
        ...
        let (head_lora_a, head_lora_b) = if let Some(num_heads) = config.num_heads {
            ...
            for _ in 0..num_heads {
                heads_a.push(Linear::new(
                    Self::kaiming_uniform_init(head_rank, head_in, dtype, &None,
                ));
                
                heads_b.push(Linear::new(
                    Tensor::zeros(Shape::from(vec![head_out, head_rank]), dtype, &None,
                ));
            }
    ...
    pub fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
        ...
        if let Some(ref gate) = self.gating {
            ...
            let gate_value = flame_core::nn::ops::sigmoid(&gate_value)?;
    ...
    fn multi_head_forward(
        ...
        let batch_dims = x.dims()[..x.rank()-1].to_vec();
        ...
        output.reshape(output_shape.dims()).map_err(|e| Error::Tensor(e.to_string()))
    }
    
    /// Apply prompt embeddings
    fn apply_prompt(&self, x: &Tensor, prompts: &Tensor) -> anyhow::Result<Tensor> {
...
impl GLoRAConv2d {
    ...
    pub fn new(
        ...
        let vb = WeightLoader::zeros(dtype, &rank,
            kernel_size,
            conv_config,
            vb.pp("lora_a"),
        )?;
        
        let lora_b = out_channels,
        ...
        // Scaling vector
        let scaling_vector = if config.use_scalar {
            Some(Tensor::ones(Shape::from(vec![rank, 1, 1]), dtype, &candle_device)?)
        } else {
            None
        };
        ...
        let gating = if config.hierarchical {
            Some(Linear::new(
                Tensor::randn(0.0f32, 0.02, Shape::from(vec![1, in_channels]), &Some(Tensor::zeros(Shape::from(vec![1]), dtype, &))
        } else {
            None
        };
        ...
        let (head_lora_a, head_lora_b) = if let Some(num_heads) = config.num_heads {
            ...
            for i in 0..num_heads {
                let vb_a = WeightLoader::zeros(dtype, &&head_rank,
                    kernel_size,
                    conv_config,
                    vb_a.pp(&format!("head_a_{}", i)),
                )?);
                
                heads_b.push(head_out,
                    1,
    ...
    pub fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
        ...
        if let (Some(ref heads_a), Some(ref heads_b)) = (&self.head_lora_a, &self.head_lora_b) {
            ...
            if let Some(ref ln) = self.layer_norm {
                ...
                result = result.transpose(1, 2)?
                    .reshape(&[shape.dim(0) * spatial, c])?;
                result = ln.forward(&result)?;
                result = result.reshape(&[shape.dim(0), spatial, c])?
                    .transpose(1, 2)?
                    .reshape(shape.dims())?;
            }
        ...
        if let Some(ref gate) = self.gating {
            ...
            let pooled = x.mean_keepdim(2isize as isize)?
                .mean_keepdim(3isize as isize)?
                .squeeze(2)?
                .squeeze(2)?;
            let gate_value = gate.forward(&pooled)?;
            let gate_value = flame_core::nn::ops::sigmoid(&gate_value)?;
            let gate_value = gate_value.unsqueeze(2)?.unsqueeze(3)?;
            result = result.mul(&gate_value)?;
        }
    ...
    fn multi_head_forward(
        &self,
        x: &Tensor,
        heads_a: &[heads_b: &[) -> anyhow::Result<Tensor> {
        ...
        for (i, (head_a, head_b)) in heads_a.iter().zip(heads_b.iter()).enumerate() {
            ...
            if let Some(ref ln) = self.layer_norm {
                ...
                head_out = head_out.transpose(1, 2)?
                    .reshape(&[h_shape.dim(0) * spatial, c])?;
                head_out = ln.forward(&head_out)?;
                head_out = head_out.reshape(&[h_shape.dim(0), spatial, c])?
                    .transpose(1, 2)?
                    .reshape(h_shape.dims())?;
            }
    ...
    pub fn initialize_layers(
        ...
        for (name, info) in model_state_dict {
            if self.should_adapt_module(name) {
                let layer = match info.shape.rank() {
                    2 => {
                        // Linear layer
                        let out_features = info.shape.dim(0);
                        let in_features = info.shape.dim(1);
                    ...
                    4 => {
                        // Conv2d layer
                        let out_channels = info.shape.dim(0);
                        let in_channels = info.shape.dim(1);
                        let kernel_size = info.shape.dim(2); // Assuming square kernel
        ...
        if state.shared_prompts.is_some() {
            if let Some(prompt_len) = self.config.prompt_length {
                let 0.02, Shape::from(vec![prompt_len, self.config.rank]), &)?;
...
impl NetworkAdapter for GLoRAAdapter {
    ...
    fn trainable_parameters(&self) -> Vec<&Parameter> {
        ...
        // TODO: Return actual Parameter references from GLoRA layers
        // For now return empty as layers use Linear/Conv2d which don't expose Parameter directly
        Vec::new()
    }
...
pub mod utils {
    ...
    pub fn analyze_component_contribution(
        ...
        sample_input: &Tensor,
    ) -> anyhow::Result<ComponentAnalysis> {
    ...
    pub fn lora_to_glora(
        ...
        config: &GLoRAConfig,
        device: &Device,
    ) -> anyhow::Result<GLoRALayer> {
        let in_features = lora_a.dim(1);
        let out_features = lora_b.dim(0);
        let rank = lora_a.dim(0);
        
        let glora = if lora_a.rank() == 2 {
            // Linear layer
