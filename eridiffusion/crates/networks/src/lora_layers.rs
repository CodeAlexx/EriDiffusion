...
//! working within Flame's tensor/backprop model while enabling proper gradient flow.
...
pub struct LinearWithLoRA {
    ...
    lora_a: Option<Parameter>,  // Down projection: [rank, in_features]
    lora_b: Option<Parameter>,  // Up projection: [out_features, rank]
...
impl LinearWithLoRA {
    ...
    pub fn new(
        ...
        let out_features = shape.dim(0);
        let in_features = shape.dim(1);
        ...
        let (lora_a, lora_b) = if let Some(r) = rank {
            ...
            let a = Parameter::from_tensor(
                &Tensor::randn(0.0, std_dev, Shape::from(vec![r, in_features]), device)?.to_dtype(weight.dtype())?
            )?;
            
            let b = Parameter::from_tensor(
                &Tensor::zeros(Shape::from(vec![out_features, r]), weight.dtype(), device)?
            )?;
    ...
    /// Create from a WeightLoader with optional LoRA
    pub fn from_vb(
        vb: WeightLoader,
        in_features: usize,
        out_features: usize,
    ...
    fn apply_dropout(&self, x: &Tensor, training: bool) -> anyhow::Result<Tensor> {
        if training && self.dropout > 0.0 {
            ...
            x.mul(&mask)?.affine(scale as f64, 0.0)
        } else {
            Ok(x.clone())
        }
    }
...
impl flame_core::Module for LinearWithLoRA {
    fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
        // Base forward pass: x @ W^T + bias
        let mut output = x.matmul(&self.weight.transpose(0, 1)?)?;
...
impl AttentionWithLoRA {
    ...
    fn reshape_for_attention(&self, x: &Tensor) -> anyhow::Result<Tensor> {
    ...
    fn attention(&self, q: &Tensor, k: &Tensor, v: &Tensor, mask: Option<&Tensor>) -> anyhow::Result<Tensor> {
        ...
        let probs = 1f32, probs.shape(), probs.device())?;
            let mask = random.ge(self.dropout)?;
            let scale = 1.0 / keep_prob;
            probs.mul(&mask)?.affine(scale as f64, 0.0)?
        } else {
            probs
        };
...
impl flame_core::Module for AttentionWithLoRA {
    fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
...
impl flame_core::Module for FeedForwardWithLoRA {
    fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
...
mod tests {
    ...
    #[test]
    fn test_linear_with_lora_forward() -> anyhow::Result<()> {
        ...
        // Create base weight
        let weight = Tensor::randn(0.0, 1.0, Shape::from(vec![out_features, in_features]), &device)?;
        
        // Create layer with LoRA
        let layer = LinearWithLoRA::new(weight.clone(), None, Some(rank), 16.0, &device)?;
        
        // Test input
        let x = Tensor::randn(0.0, 1.0, Shape::from(vec![batch_size, seq_len, in_features]), &device)?;
    ...
    #[test]
    fn test_lora_gradient_flow() -> anyhow::Result<()> {
        let device = Device::cuda(0)?;
        let layer = LinearWithLoRA::new(
            Tensor::randn(0.0, 1.0, Shape::from(vec![256, 256]), &device)?,
            None,
            Some(8),
            8.0,
            &device,
        ...
        let x = Tensor::randn(0.0, 1.0, Shape::from(vec![1, 10, 256]), &device)?;
        let output = layer.forward(&x)?;
        
        // Create a dummy loss
        let target = Tensor::randn(0.0, 1.0, Shape::from(vec![1, 10, 256]), &device)?;
        ...
        for param in layer.trainable_parameters() {
            assert!(param.gradient().is_ok(), "LoRA parameter missing gradient");
        }
    ...
    #[test]
    fn test_attention_with_lora() -> anyhow::Result<()> {
        let device = Device::cuda(0)?;
        let dim = 512;
        let num_heads = 8;
        ...
        // Test forward pass
        let x = Tensor::randn(0.0, 1.0, Shape::from(vec![2, 10, dim]), &device)?;
