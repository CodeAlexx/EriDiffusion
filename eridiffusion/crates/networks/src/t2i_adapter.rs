...
impl InstanceNorm {
    fn new(num_features: usize, device: &weight: Tensor::ones(Shape::from(vec![num_features]), DType::F32, device)?,
            bias: Tensor::zeros(Shape::from(vec![num_features]), DType::F32, device)?,
            eps: 1e-5,
        })
    }
    ...
    fn forward(&self, x: &Tensor) -> anyhow::Result<Tensor> {
        ...
        let mean = x_reshaped.mean_keepdim(2isize as isize)?;
        let var = x_reshaped.var_keepdim(2isize)?;
...
impl DownBlock {
    ...
    fn forward(&self, x: &Tensor) -> anyhow::Result<(Tensor, Vec<Tensor>)> {
        let mut h = x.clone();
...
impl T2IAdapter {
    ...
    pub fn new(config: T2IAdapterConfig, device: Device) -> anyhow::Result<Self> {
        let config.channels[0],
            3,
            Conv2dConfig {
                padding: 1,
                stride: 1,
    ...
    pub fn extract_features(&self, condition: &Tensor) -> anyhow::Result<Vec<Tensor>> {
        ...
        for (i, block) in state.down_blocks.iter().enumerate() {
            ...
            if i < state.feature_extractors.len() {
                ...
                let last_feature = block_features.last()
                    .ok_or_else(|| Error::Network("block_features cannot be empty".into()))?;
    ...
    pub fn for_condition_type(condition_type: &str, device: Device) -> anyhow::Result<Self> {
...
impl NetworkAdapter for T2IAdapter {
    fn forward(&self, x: &Tensor, inputs: &eridiffusion_core::ModelInputs) -> anyhow::Result<NetworkOutput> {
        ...
        let condition = inputs.additional.get("t2i_condition")
            .ok_or_else(|| Error::Model("Missing T2I-Adapter conditioning".into()))?;
    ...
    fn merge_adapters(&mut self, _adapters: Vec<Box<dyn NetworkAdapter>>, _weights: Vec<f32>) -> anyhow::Result<()> {
        ...
        Err(Error::Model("T2I-Adapter does not support adapter merging".into()))
    }
}
*/
...
pub mod utils {
    ...
    fn preprocess_sketch(sketch: &Tensor, target_size: (usize, usize)) -> anyhow::Result<Tensor> {
        // Convert to grayscale if needed
        let gray = if sketch.dim(1) == 3 {
    ...
    fn preprocess_depth(depth: &Tensor, target_size: (usize, usize)) -> anyhow::Result<Tensor> {
    ...
    fn preprocess_segmentation(seg: &Tensor, target_size: (usize, usize)) -> anyhow::Result<Tensor> {
        // Segmentation maps are usually already in the right format
        // Just ensure correct number of channels
        Ok(seg.clone())
    }
    
    fn preprocess_openpose(pose: &Tensor, target_size: (usize, usize)) -> anyhow::Result<Tensor> {
        // OpenPose keypoints are typically already processed
        Ok(pose.clone())
    }
    
    fn preprocess_color(color: &Tensor, target_size: (usize, usize)) -> anyhow::Result<Tensor> {
