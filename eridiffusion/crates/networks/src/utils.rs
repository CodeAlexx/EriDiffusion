...
pub mod adapter {
    ...
    pub fn merge_adapters(
        ...
        weights: Vec<f32>,
    ) -> anyhow::Result<Box<dyn NetworkAdapter + Send + Sync>> {
        if adapters.is_empty() {
            return Err(Error::Model("No adapters to merge".into()));
        }
        
        if adapters.len() != weights.len() {
            return Err(Error::Model("Adapter and weight counts must match".into()));
        }
        ...
        let mut merged = adapters.into_iter().next()
            .ok_or_else(|| Error::Network("Cannot merge empty adapter list".into()))?;
    ...
    pub async fn load_adapter_from_checkpoint(
        ...
        let tensors = flame_core::safetensors::load(checkpoint_path, device)?;
        ...
        if is_lora {
            ...
            Err(Error::Model("Unknown adapter type in checkpoint".into()))
        }
    }
}
...
pub mod quantization {
    ...
    /// Dequantize tensor
    pub fn dequantize_tensor(quantized: &QuantizedTensor) -> anyhow::Result<Tensor> {
    ...
    fn compute_quantization_params(
        ...
        // Compute min/max
        let min_val = tensor.min_keepdim(0isize)?;
        let max_val = tensor.max_keepdim(0isize)?;
        ...
        let zero_point = if config.symmetric {
            ...
            (Tensor::from_vec(&[qmin], scale.device())? - (&min_val / &scale)?)?
        };
...
pub mod pruning {
    ...
    fn unstructured_magnitude_prune(tensor: &Tensor, sparsity: f32) -> anyhow::Result<Tensor> {
        // Compute magnitude
        let magnitude = tensor.abs()?;
        
        // Flatten and sort
        let flat_mag = magnitude.flatten_all()?;
        let num_elements = flat_mag.dim(0);
    ...
    fn structured_magnitude_prune(tensor: &Tensor, sparsity: f32) -> anyhow::Result<Tensor> {
        // Compute channel-wise magnitude
        let channel_mag = tensor.sqr()?.sum_keepdim(1isize as isize)?.sqrt()?;
        
        // Determine channels to prune
        let num_channels = tensor.dim(0);
...
pub mod conversion {
    ...
    pub fn convert_adapter(
        ...
        device: &Device,
    ) -> anyhow::Result<Box<dyn NetworkAdapter>> {
        use NetworkType::*;
    ...
    pub async fn export_to_onnx(
        ...
        // 1. Convert Flame tensors to ONNX graph representation
        // 2. Handle operator mapping from Flame to ONNX
