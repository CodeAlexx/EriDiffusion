...
use eridiffusion_core::{DType as CoreDType, Device};
use eridiffusion_training::{gradient_accumulator::*, loss::*, optimizer::*};
...
fn benchmark_loss_functions(c: &mut Criterion) {
    ...
    for (h, w) in sizes {
        let pred = Tensor::randn(0f32, 1f32, Shape::from(vec![1, 3, h, w]), &device).unwrap();
        let target = Tensor::randn(0f32, 1f32, Shape::from(vec![1, 3, h, w]), &device).unwrap();

        let config = LossConfig { weight: 1.0, reduction: ReductionType::Mean };
        ...
        group.bench_with_input(
            ...
            |b, (pred, target)| {
                ...
                b.iter(|| black_box(loss.compute(pred, target).unwrap()));
            },
        );
...
fn benchmark_gradient_accumulation(c: &mut Criterion) {
    ...
    let param_sizes =
        vec![("small", vec![512, 512]), ("medium", vec![2048, 2048]), ("large", vec![4096, 4096])];

    for (name, shape) in param_sizes {
        let param = Tensor::randn(0f32, 1f32, &shape, device).unwrap();
        let params = vec![&param];

        group.bench_function(BenchmarkId::new("accumulate", name), |b| {
...
fn benchmark_gradient_clipping(c: &mut Criterion) {
    ...
    for n in num_params {
        ...
        for _ in 0..n {
            gradients
                .push(Tensor::randn(0f32, 1f32, Shape::from(vec![128, 128]), &device).unwrap());
        }

        group.bench_function(BenchmarkId::new("global_norm", n), |b| {
...
fn benchmark_optimizer_step(c: &mut Criterion) {
    ...
    let param_shapes =
        vec![("small", vec![128, 128]), ("medium", vec![512, 512]), ("large", vec![1024, 1024])];

    for (name, shape) in param_shapes {
        let params: Vec<Tensor> =
            (0..10).map(|_| Tensor::randn(0f32, 1f32, &shape, device).unwrap()).collect();
        let param_refs: Vec<&Tensor> = params.iter().collect();

        let grads: Vec<Tensor> =
            (0..10).map(|_| Tensor::randn(0f32, 0.01f32, &shape, device).unwrap()).collect();
        ...
        group.bench_function(BenchmarkId::new("adam", name), |b| {
            ...
            b.iter(|| {
                ...
                // TODO: Use gradient_map instead of individual tensor
            });
        });
    }
