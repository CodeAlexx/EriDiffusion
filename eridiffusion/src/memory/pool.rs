use super::cuda_allocator::CudaAllocator;
use super::{DiffusionConfig, MemoryFormat, MemoryPoolConfig, PrecisionMode};
...
pub struct MemoryPool {
    ...
    allocated_blocks: RwLock<std::collections::HashMap<*mut c_void, MemoryBlock>>,
    cached_blocks: RwLock<Vec<MemoryBlock>>,
    attention_cache: RwLock<std::collections::HashMap<(usize, usize), *mut c_void>>,
    gradient_blocks: RwLock<std::collections::HashMap<*mut c_void, MemoryBlock>>,
...
impl MemoryPool {
    pub fn new(device_id: i32, config: MemoryPoolConfig) -> flame_core::Result<Self> {
    ...
    pub fn configure_for_diffusion(
        &mut self,
    ...
    pub fn allocate_for_attention(
        &self,
        seq_len: usize,
        batch_size: usize,
        head_dim: usize,
    ) -> flame_core::Result<*mut c_void> {
        let key = (seq_len, batch_size);
    ...
    pub fn clear_gradients(&self) -> flame_core::Result<()> {
        let gradient_blocks = {
    ...
    pub fn empty_cache(&self) -> flame_core::Result<()> {
        let mut cached = self.cached_blocks.write().unwrap();
    ...
    pub fn get_recommended_batch_size(&self, per_sample_memory: usize) -> usize {
        // TODO: Get actual memory info from allocator
        let free = 1024 * 1024 * 1024; // 1GB placeholder
    ...
    pub fn get_memory_pressure(&self) -> f64 {
        // TODO: Get actual memory info from allocator
        let (free, total) = (1024_u64 * 1024 * 1024, 2_u64 * 1024 * 1024 * 1024); // Placeholder
        let used = total - free;
        used as f64 / total as f64
    }

    pub fn get_memory_info(&self) -> flame_core::Result<String> {
        let stats = self.get_stats();
        Ok(format!(
            "Allocated: {} MB, Cached: {} MB, Hit rate: {:.2}%",
            stats.allocated_bytes / 1048576,
            stats.cached_bytes / 1048576,
    ...
    fn update_stats_allocation(&self, size: usize) -> flame_core::Result<()> {
        let mut stats = self.stats.write().unwrap();
    ...
    fn update_stats_cache_hit(&self, size: usize) -> flame_core::Result<()> {
        let mut stats = self.stats.write().unwrap();
    ...
    fn update_stats_deallocation(&self, size: usize) -> flame_core::Result<()> {
        let mut stats = self.stats.write().unwrap();
    ...
    fn maybe_garbage_collect(&self) -> flame_core::Result<()> {
        let (allocated, threshold) = {
            let stats = self.stats.read().unwrap();
            (
                stats.allocated_bytes + stats.cached_bytes,
                (self.config.max_size as f64 * self.config.garbage_collection_threshold as f64)
                    as usize,
            )
        };
    ...
    pub fn garbage_collect(&self) -> flame_core::Result<()> {
        let mut cached = self.cached_blocks.write().unwrap();
        let mut to_remove = Vec::new();

        cached.sort_by_key(|block| block.last_accessed);

        let target_size = (self.config.max_size as f64
            * self.config.garbage_collection_threshold as f64
            * 0.8) as usize;
        ...
        println!("Garbage collected {} blocks", to_remove.len());
