...
pub mod cuda {
    ...
    pub fn device_count() -> Result<usize, cuda_allocator::MemoryError> {
        // For now, assume 1 device when CUDA_VISIBLE_DEVICES=0
        // In a full implementation, this would query cudaGetDeviceCount
        if std::env::var("CUDA_VISIBLE_DEVICES").as_deref() == Ok("0") {
            Ok(1)
        } else {
            Ok(8) // Assume 8 devices if not restricted
        }
    }
}