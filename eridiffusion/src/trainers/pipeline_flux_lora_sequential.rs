...
use flame_core::{Tensor, DType, Parameter, Result, Shape};
...
use crate::trainers::{
    ...
    pipeline_flux_lora::{
        FluxTrainingConfig, TextEncoderPaths,
        FluxLoRALayer, TrainingBatch, FluxNoiseScheduler,
        PredictionType, TrainMode,
    },
    ...
    pipeline_progress_tracker::{ProgressTracker, MemoryTracker},
    memory_optimizer::{MemoryOptimizer, MemoryStrategy},
    checkpoint_saver::{CheckpointSaver, CheckpointMetadata},
};
use std::sync::Arc;
...
impl FluxTrainerSequential {
    ...
    pub fn new_with_sequential_loading(
        ...
        println!("This implementation properly loads models in sequence to maximize memory efficiency");
        ...
        if let Some(ref cache_mgr) = cache_manager {
            ...
            if need_text {
                ...
                if missing_count == 0 && !config.force_reencode {
                    ...
                    println!("  Found {} missing embeddings in first {} samples", missing_count, check_limit);
        ...
        let (flux_with_lora, flux_base, streaming_flux, lora_layers) = if use_int8 {
            ...
            if let Some(chroma_config) = &config.chroma_config {
                ...
                println!("  ChromaXL pattern: {} (will be loaded on-demand)", chroma_config.layer_pattern);
                ...
                println!("  Standard Flux LoRA training (streaming mode - layers loaded on-demand)");
            ...
            let flux = load_flux_with_cpu_offloading(&config.model_path, &device, &model_offloader)?;
        ...
        let noise_scheduler = FluxNoiseScheduler::new(
            device.clone(),
            config.shift_schedule,
        );
            ...
            while let Some(batch) = data_loader.next_batch_old()? {
                ...
                if step % self.config.gradient_accumulation_steps == 0 {
                    ...
                    if let Ok(mem_mb) = self.memory_tracker.get_memory_usage_mb() {
                        ...
                        let mem_stats = self.memory_optimizer.get_stats();
                        if mem_stats.pressure as u8 >= crate::trainers::memory_optimizer::MemoryPressure::High as u8 {
                            let tips = crate::trainers::memory_optimizer::get_optimization_tips(&mem_stats);
    ...
    fn training_step(&mut self, batch: TrainingBatch) -> flame_core::Result<f32> {
        ...
        let mut latents = if let Some(ref cache_manager) = self.cache_manager {
            ...
            cache_manager.load_latent(image_path)?
                .ok_or_else(|| flame_core::Error::InvalidOperation(
                    format!("Latent not found in cache for: {:?}", image_path)
                ))?
        } else {
        ...
        let mut latents = patchify_latents(&latents)?
        ...
        let (clip_hidden_states, t5_embeds) = if let Some(ref cache_manager) = self.cache_manager {
            let image_path = &batch.image_paths[0];
            let (clip, t5) = cache_manager.load_text_embeddings(image_path)?
                .ok_or_else(|| flame_core::Error::InvalidOperation(
                    format!("Text embeddings not found in cache for: {:?}", image_path)
                ))?;
        ...
        let img_ids = Tensor::from_slice(&img_ids_vec, Shape::from_dims(&[1, h * w, 2]), noisy_latents.device().clone())?
            .to_dtype(flame_core::DType::BF16)?;
        ...
        let txt_ids = Tensor::from_slice(&txt_ids_vec, Shape::from_dims(&[1, txt_seq_len]), noisy_latents.device().clone())?
            .to_dtype(flame_core::DType::BF16)?;
        ...
        print!("\rStep {}/{} - Running Flux model (this takes time on first step)... ", 
            self.global_step + 1, 
            self.config.max_train_steps);
        std::io::Write::flush(&mut std::io::stdout()).ok();
        ...
        let model_pred = if let Some(streaming_flux) = &mut self.streaming_flux {
            ...
            debug_assert!({
                ...
                let is_reasonable = out_max.is_finite() && out_min.is_finite() && 
                                   out_max.abs() < 1e10 && out_min.abs() < 1e10;
                if !is_reasonable && self.global_step == 0 {
                    eprintln!("🚨 Model output explosion detected: [{:.2e}, {:.2e}]", out_min, out_max);
                }
                is_reasonable
        ...
        print!("\rStep {}/{} - Computing loss and gradients... ", 
            self.global_step + 1, 
            self.config.max_train_steps);
        std::io::Write::flush(&mut std::io::stdout()).ok();
        ...
        if !loss_value.is_finite() {
            eprintln!("⚠️ Warning: NaN/inf loss detected at step {}, skipping batch", self.global_step);
    ...
    fn sample_timesteps(&self, batch_size: usize) -> flame_core::Result<Tensor> {
        ...
        let timesteps = (0..batch_size)
            .map(|_| rand::thread_rng().gen::<f32>() * 1000.0)
            .collect::<Vec<_>>();
    ...
    fn validate(&mut self) -> flame_core::Result<()> {
        ...
        if let Some(val_batch) = &self.validation_batch {
            ...
            let latents = val_batch.pixel_values.as_ref()
                .ok_or_else(|| flame_core::Error::InvalidOperation("Validation batch missing pixel_values".into()))?;
            let encoder_hidden_states = val_batch.encoder_hidden_states.as_ref()
                .ok_or_else(|| flame_core::Error::InvalidOperation("Validation batch missing encoder_hidden_states".into()))?;
            let pooled_embeds = val_batch.pooled_encoder_hidden_states.as_ref()
                .ok_or_else(|| flame_core::Error::InvalidOperation("Validation batch missing pooled_encoder_hidden_states".into()))?;
            ...
            println!("  Model predictions - min: {:.4}, max: {:.4}, mean: {:.4}", 
    ...
    fn validate_gradient_targets(&self) -> flame_core::Result<()> {
        ...
        if let Some(lora_layers) = &self.lora_layers {
            ...
            if trainable_count != expected_trainable {
                warn!("⚠️ Expected {} trainable parameters, found {}", expected_trainable, trainable_count);
            }
        }
    ...
    fn save_checkpoint(&mut self) -> flame_core::Result<()> {
        ...
        if let Some(lora_layers) = &self.lora_layers {
            for (name, layer) in lora_layers {
                ...
                if layer.scale != 1.0 {
                    let scale_tensor = Tensor::from_scalar(layer.scale, self.device.cuda_device_arc())?;
                    model_weights.insert(
                        format!("{}.scale", name),
                        scale_tensor
                    );
        ...
        metadata.insert("flux_model_path".to_string(), self.config.model_path.to_string_lossy().to_string());
    ...
    pub fn resume_from_checkpoint(&mut self, checkpoint_path: &Path) -> flame_core::Result<()> {
            ...
            for (key, tensor) in opt_state {
                if let Some(param_name) = key.strip_prefix("optimizer.") {
                    if let Some(tensor_type) = param_name.split('.').last() {
                        ...
                        match tensor_type {
                            "m" => {
                                optimizer_tensors.entry(param_key.to_string())
                                    .or_insert((None, None)).0 = Some(tensor);
                            }
                            "v" => {
                                optimizer_tensors.entry(param_key.to_string())
                                    .or_insert((None, None)).1 = Some(tensor);
                            }
                            _ => {}
                        }
                    }
        ...
        println!("Resumed from step {} (epoch {}, best loss: {:.6})", 
    ...
    pub fn add_noise(&self, latents: &Tensor, noise: &Tensor, timesteps: &Tensor) -> flame_core::Result<Tensor> {
                    ...
                    for c in 0..channels {
                        for dy in 0..patch_size {
                            for dx in 0..patch_size {
                                ...
                                let src_idx = b * channels * height * width + c * height * width + y * width + x;
...
fn load_flux_with_cpu_offloading(
    ...
    println!("  [LOAD_FLUX] ✅ WeightLoader returned successfully with {} weights in {:.1}s", 
        weight_loader.weights.len(), start_time.elapsed().as_secs_f32());
    ...
    println!("  [LOAD_FLUX] ✅ Model loading complete in {:.1}s", start_time.elapsed().as_secs_f32());
    Ok(model)
}
...
fn setup_flux_lora(
    ...
    for module_name in target_modules {
        let (in_features, out_features) = match module_name.as_str() {
            ...
            "img_mlp" | "txt_mlp" => {
                let lora1 = FluxLoRALayer::new(hidden_size, mlp_hidden, rank, alpha, dropout, device)?;
                lora_layers.insert(format!("{}.lin1", module_name), lora1);
                
                let lora2 = FluxLoRALayer::new(mlp_hidden, hidden_size, rank, alpha, dropout, device)?;
