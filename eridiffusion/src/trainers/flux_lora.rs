...
impl FluxLoRAConfig {
    pub fn from_process_config(config: &ProcessConfig, device: Device) -> Result<Self> {
        ...
        Ok(Self {
            ...
            vae_tiling: false,  // TODO: implement VAE tiling
            cache_latents: config.datasets[0].cache_latents_to_disk.unwrap_or(true),
            enable_block_swapping: true,  // Enable for 24GB GPUs
            output_dir: PathBuf::from(&config.save.save_every.to_string()),
            device,
        })
    }
}
...
pub struct FluxLoRATrainer {
    ...
    // CPU offloaded model wrapper
    cpu_offloaded_model: Option<crate::trainers::flux_cpu_offloaded_model::CpuOffloadedFluxModel>,
...
impl FluxLoRATrainer {
    ...
    pub fn train_step_cached(
        ...
        let output_patches = if let Some(cpu_offloaded_model) = &self.cpu_offloaded_model {
            ...
            match cpu_offloaded_model.forward(
                ...
                Ok(out) => out,
                Err(e) => {
                    println!("Error in cpu_offloaded_model.forward: {:?}", e);
        ...
        // 8. Backward pass (accumulate gradients)
        ...
        // 9. Optimizer step only after accumulating enough gradients
    ...
    pub fn save_checkpoint(&self, step: usize) -> Result<()> {
        let checkpoint_path = self.config.output_dir.join(format!("flux_lora_step_{}.safetensors", step));
        ...
        let state_path = self.config.output_dir.join(format!("flux_lora_state_{}.json", step));
...
pub fn train_flux_lora(config: &Config, process_config: &ProcessConfig) -> Result<()> {
    ...
    let data_config = DatasetConfig {
        ...
        shuffle_tokens: dataset_config.shuffle_tokens.unwrap_or(false),
        cache_latents_to_disk: dataset_config.cache_latents_to_disk.unwrap_or(true),
