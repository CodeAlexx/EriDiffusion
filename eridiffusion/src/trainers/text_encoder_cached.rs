...
impl CachedTextEncoder {
    ...
    fn tokenize_clip(&self, tokenizer: &Tokenizer, text: &str, max_length: usize) -> Result<Tensor> {
    ...
    fn tokenize_t5(&self, tokenizer: &Tokenizer, text: &str, max_length: usize) -> Result<Tensor> {
        let encoding = tokenizer.encode(text, true)
            .map_err(|e| anyhow::anyhow!("Tokenization failed: {:?}", e))?;
        let mut ids = encoding.get_ids().to_vec();
