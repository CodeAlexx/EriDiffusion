...
use flame_core::{Tensor, Shape, DType, Result, Parameter};
...
use crate::inference::flux::{FluxInference, FluxConfig as InferenceConfig};
...
impl FluxLoRALayer {
    ...
    pub fn forward(&self, x: &Tensor, base_output: &Tensor, training: bool) -> flame_core::Result<Tensor> {
        ...
        let lora_out = if self.dropout > 0.0 && training {
            ...
            let dropout_mask = Tensor::randn(x.shape().clone(), 0.5, 0.289, x.device().clone())?.add_scalar(0.5)?;
            // Create a mask where values < (1.0 - dropout) are 1.0, else 0.0
            let threshold = Tensor::full(dropout_mask.shape().clone(), 1.0 - self.dropout, dropout_mask.device().clone())?;
        ...
        debug_assert!({
            let delta_mean = scaled_lora.abs()?.mean()?.to_scalar::<f32>().unwrap_or(f32::INFINITY);
            if delta_mean > 100.0 {
                eprintln!("❌ LoRA delta too large, possible misinit or accumulation: mean={:.2e}", delta_mean);
                false
            } else {
                true
            }
        ...
        debug_assert!({
            ...
            base_max.is_finite() && base_min.is_finite() && base_max.abs() < 1e6 && base_min.abs() < 1e6
        ...
        debug_assert!({
            ...
            lora_max.is_finite() && lora_min.is_finite() && lora_max.abs() < 100.0 && lora_min.abs() < 100.0
    ...
    pub fn add_noise(
        ...
        for i in 0..batch_size {
            ...
            let noise_slice = if noise_dims.len() == 3 {
                ...
                // 4D tensor: [batch, channels, height, width]
                noise.slice(&[(i, i + 1), (0, noise_dims[1]), (0, noise_dims[2]), (0, noise_dims[3])])?
            };
...
pub trait DataLoader: Iterator<Item = Result<TrainingBatch>> {
    ...
    fn enumerate(self) -> std::iter::Enumerate<Self> where Self: Sized {
...
impl FluxTrainer {
    ...
    pub fn new(config: FluxTrainingConfig, device: Device) -> flame_core::Result<Self> {
        ...
        let cache_manager = if config.cache_latents_to_disk {
            let cache_dir = config.cache_dir.clone()
                .unwrap_or_else(|| config.logging_dir.join("cache"));
        ...
        let (vae, text_encoders) = if config.cache_latents_to_disk {
            ...
            let text_encoders = if skip_text {
                ...
                let encoders = Some(Arc::new(load_text_encoders(&config.text_encoder_paths, &device)?));
        ...
        // Gradient accumulator
        let gradient_accumulator = GradientAccumulator::new(config.gradient_accumulation_steps, device.clone());
    ...
    pub fn pre_encode_data(&mut self, data_loader: &mut FluxDataLoader) -> flame_core::Result<()> {
        if let Some(ref cache_manager) = self.cache_manager {
            ...
            // Clear memory pool
            if let Ok(pool) = flame_core::memory_pool::MEMORY_POOL.get_pool(&self.device.cuda_device_arc()) {
    ...
    fn load_flux_model_for_training(&mut self) -> flame_core::Result<()> {
        ...
        let trainable_params = get_trainable_parameters(&self.flux_base, &self.lora_layers, &self.config);
    ...
    pub fn train(&mut self, data_loader: &mut FluxDataLoader) -> flame_core::Result<()> {
        ...
        if self.config.gradient_checkpointing {
            ...
            let mut checkpoint_manager = flame_core::gradient_checkpointing::CHECKPOINT_MANAGER.lock().unwrap();
            
            // Use CPU offload for better accuracy preservation during training
            checkpoint_manager.set_policy(flame_core::gradient_checkpointing::CheckpointPolicy::CPUOffload);
        ...
        for epoch in 0..self.config.max_train_steps / num_update_steps_per_epoch + 1 {
            ...
            println!("\n========== Epoch {}/{} ==========", epoch + 1, self.config.max_train_steps / num_update_steps_per_epoch + 1);
            ...
            while let Some(batch) = data_loader.next_batch_old()? {
                ...
                if step % self.config.gradient_accumulation_steps == 0 {
                    ...
                    // Calculate progress
                    let progress = (self.global_step as f32 / self.config.max_train_steps as f32) * 100.0;
                    ...
                    let grad_norm = if let Some(lora_layers) = &self.lora_layers {
                        ...
                        if count > 0 { total_norm / count as f32 } else { 0.0 }
                    } else {
                        0.0
                    };
    ...
    fn training_step(&mut self, batch: TrainingBatch) -> flame_core::Result<f32> {
        ...
        println!("  [training_step] Batch loaded: {} images, {} prompts", 
                 images.shape().dims()[0], prompts.len());
        ...
        let latents = if let Some(ref cache_manager) = self.cache_manager {
            ...
            if let Some(cached_latent) = cache_manager.load_latent(image_path)? {
                println!("  [training_step] ✅ Loaded cached latent for: {:?}", image_path.file_name().unwrap_or_default());
                cached_latent
            } else {
                // Fallback to encoding if not cached
                println!("  [training_step] ⚠️  Cache miss for: {:?}, falling back to VAE encoding", image_path.file_name().unwrap_or_default());
        ...
        // Add noise using flow matching
        let (noisy_latents, velocity) = self.noise_scheduler.add_noise(&latents, &noise, &timesteps)?;
        ...
        let (clip_embeds, t5_embeds) = if let Some(ref cache_manager) = self.cache_manager {
            ...
            if let Some((clip_cached, t5_cached)) = cache_manager.load_text_embeddings(image_path)? {
                println!("  [training_step] ✅ Loaded cached text embeddings for: {:?}", image_path.file_name().unwrap_or_default());
                (clip_cached, t5_cached)
            } else {
                println!("  [training_step] ⚠️  Cache miss for text embeddings: {:?}", image_path.file_name().unwrap_or_default());
                // Fallback to encoding if not cached
        ...
        let guidance_tensor = if !self.config.bypass_guidance_embedding {
            Some(Tensor::full(timesteps.shape().clone(), self.config.guidance_scale, timesteps.device().clone())?)
        } else {
            None
        };
        ...
        let model_pred = if let Some(flux_with_lora) = &self.flux_with_lora {
            ...
            return Err(flame_core::Error::InvalidOperation("No model available for forward pass".into()));
        };
        ...
        let loss = match self.noise_scheduler.prediction_type {
            PredictionType::VPrediction => {
                mse_loss(&model_pred, &velocity)?
            }
            PredictionType::Epsilon => {
                mse_loss(&model_pred, &noise)?
            }
        };
    ...
    fn encode_prompts(&self, prompts: &[String]) -> flame_core::Result<(Tensor, Tensor)> {
        let text_encoders = self.text_encoders.as_ref()
            .ok_or_else(|| flame_core::Error::InvalidOperation("Text encoders not loaded".into()))?;
    ...
    pub fn generate_sample(&self, prompt: &str, seed: Option<u64>) -> flame_core::Result<Tensor> {
        ...
        let model_config = ModelConfig {
            ...
            clip2_path: None,
            t5_path: self.config.text_encoder_paths.t5_xxl.as_ref().map(|p| p.to_string_lossy().to_string()),
    ...
    fn save_checkpoint(&self) -> flame_core::Result<()> {
        let checkpoint_path = self.config.logging_dir.join(format!("checkpoint-{}", self.global_step));
        std::fs::create_dir_all(&checkpoint_path).map_err(|e| flame_core::Error::InvalidOperation(format!("Failed to create directory: {}", e))).map_err(|e| flame_core::Error::InvalidOperation(e.to_string()))?;
    ...
    fn save_image(&self, image: &Tensor, filename: &str) -> flame_core::Result<()> {
        ...
        std::fs::create_dir_all(path.parent().unwrap()).map_err(|e| flame_core::Error::InvalidOperation(e.to_string()))?;
...
/// Create optimizer
fn create_optimizer(params: Vec<Parameter>, config: &FluxTrainingConfig) -> flame_core::Result<Adam8bit> {
...
fn load_vae(path: &Path, device: &Device) -> flame_core::Result<AutoencoderKL> {
    ...
    let weight_loader = WeightLoader::from_safetensors_with_dtype(path, device.clone(), DType::F16)?;
...
fn load_flux_model(path: &Path, device: &Device) -> flame_core::Result<FluxModel> {
    ...
    if let Ok(metadata) = std::fs::metadata(path) {
        ...
        if size_gb > 20.0 {
            println!("  ⚠️  WARNING: Model is very large ({:.1} GB), may cause OOM in 24GB VRAM", size_gb);
    ...
    let weight_loader = WeightLoader::from_safetensors_streaming(path, device.clone(), DType::BF16)?;
    println!("  ✅ Weights loaded via memory mapping, total tensors: {}", weight_loader.weights.len());
    
    // Detect model variant from weights
    let config = if weight_loader.weights.contains_key("model.diffusion_model.double_blocks.0.txt_mlp.0.weight") {
...
fn load_text_encoders(paths: &TextEncoderPaths, device: &Device) -> flame_core::Result<TextEncoders> {
...
fn save_lora_weights(lora_layers: &HashMap<String, FluxLoRALayer>, path: &Path) -> flame_core::Result<()> {
    use safetensors::{tensor::Dtype as SafeDtype, serialize};
    ...
    // Write to file
    std::fs::write(path.join("flux_lora.safetensors"), data).map_err(|e| flame_core::Error::InvalidOperation(e.to_string()))?;
...
fn save_training_state(state: &TrainingState, path: &Path) -> flame_core::Result<()> {
    let json = serde_json::to_string_pretty(state)
        .map_err(|e| flame_core::Error::InvalidOperation(format!("Failed to serialize state: {}", e)))?;
    
    std::fs::write(path.join("training_state.json"), json).map_err(|e| flame_core::Error::InvalidOperation(e.to_string()))?;
    Ok(())
}
...
fn save_tensor_as_image(tensor: &Tensor, path: &Path) -> flame_core::Result<()> {
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| flame_core::Error::InvalidOperation(format!("Failed to create directory: {}", e))).map_err(|e| flame_core::Error::InvalidOperation(e.to_string()))?;
    }
