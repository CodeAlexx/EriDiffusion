...
use crate::config::trainer_config::{ProcessConfig, TrainConfig, TrainingConfig};
use crate::models::flux_vae::{AutoEncoderConfig as VAEConfig, AutoencoderKL};
use crate::models::{FluxModel, FluxModelConfig};
use crate::trainers::flux_data_loader::FluxDataLoader;
use crate::trainers::{
    flux_cache_manager::FluxCacheManager, streaming_text_encoders::StreamingTextEncoders,
    text_embedding_cache::PersistentEmbeddingCache,
};
use flame_core::{CudaDevice, DType, Device, Module, Result, Tensor};
...
pub fn train_flux_lora_sequential_fixed(config: TrainingConfig) -> Result<()> {
    ...
    println!(
        "  Image size: {}x{}",
        process_config.datasets[0].resolution[0], process_config.datasets[0].resolution[0]
    );
    ...
    if needs_processing {
        ...
        if missing_latents > 0 {
            ...
            let vae_path = process_config.model.vae_path.as_ref().unwrap_or(&default_vae_path);
            ...
            cache_mgr.encode_all_latents_with_model(
                &mut data_loader,
                vae_ref,
                process_config.datasets[0].force_recache,
            )?;
        ...
        if missing_text > 0 {
            ...
            let mut text_cache =
                PersistentEmbeddingCache::new(text_cache_dir, device_flame.clone())?;
