...
/// Extended Prodigy wrapper for integration
pub enum ProdigyOptimizer {
    Standard(Prodigy),
}

impl CanOptimizer for ProdigyOptimizer {
    type Config = ProdigyConfig;
...
/// Create Prodigy optimizer from config
pub fn create_prodigy_optimizer(
    vars: Vec<Var>,
    config: Option<ProdigyConfig>,
) -> eridiffusion_core::Result<ProdigyOptimizer> {
    let config = config.unwrap_or_default();
    ProdigyOptimizer::new(vars, config)
        .map_err(|e| Error::Config(format!("Failed to create Prodigy optimizer: {}", e)))
}
...
pub fn create_prodigy_preset(
    ...
    let config = match preset {
        ...
        _ => return Err(Error::Config(format!("Unknown Prodigy preset: {}", preset))),
    };
    
    create_prodigy_optimizer(vars, Some(config))
}
